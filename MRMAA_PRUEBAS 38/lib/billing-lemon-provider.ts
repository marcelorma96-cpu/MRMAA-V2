...
export async function checkout(req: NextRequest, input: BillingInput) {
  ...
  try {
    ...
    const response = await lemonRequest("/checkouts", { data: { type: "checkouts", attributes: {
      product_options: { enabled_variants: [Number(variant)], redirect_url: `${origin}/?billing=success`, receipt_link_url: `${origin}/?billing=return`, receipt_button_text: "UnoMesa" },
...
export async function change(req: NextRequest, input: BillingInput) {
  ...
  try {
    ...
    if (String(current.variant_id) !== binding.variant_id || currentOffer.plan !== restaurant.plan_code || currentOffer.interval !== restaurant.billing_cycle)
      throw new BillingError("El proveedor y UnoMesa todavía no muestran el mismo plan. Espere la sincronización y actualice; si persiste, contacte a soporte.", 409, "SUBSCRIPTION_NOT_SYNCED");
    ...
    const translations: Record<string, string> = {
      ...
      SUBSCRIPTION_NOT_SYNCED: "The provider and UnoMesa do not show the same plan yet. Wait for synchronization and refresh. Contact support if it persists.",
...
export async function resume(req: NextRequest, input: BillingInput) {
  ...
  try {
    ...
    // Signed webhooks remain the only path that updates paid access in UnoMesa.
...
export async function webhook(req: NextRequest) {
  ...
  try {
    ...
    // Ignore unrelated store orders; initial UnoMesa subscription events must have
