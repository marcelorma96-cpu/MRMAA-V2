...
import { formatAppMoney } from "@/components/app-preferences";
...
export function ReportsModule({
  ...
  function printReport() {
    printHtml(
      `<html><head><title>Reporte UnoMesa</title><style>body{font-family:Arial;padding:30px;color:#18181b}h1{font-family:Georgia}.cards{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}.cards div{border:1px solid #ddd;padding:18px}.cards b,.cards span{display:block}.cards b{font-size:24px;margin-top:6px}table{width:100%;border-collapse:collapse;margin-top:25px}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{background:#18181b;color:#fff}</style></head><body><h1>Reporte operativo</h1><p>${from} a ${to}</p><section class="cards"><div><span>Reservaciones</span><b>${filtered.length}</b><small>${confirmed} confirmadas</small></div><div><span>Invitados</span><b>${guests}</b></div><div><span>Valor cotizado</span><b>${money(total)}</b></div><div><span>Anticipos</span><b>${money(deposits)}</b></div></section><table><tr><th>Fecha</th><th>Cliente</th><th>Invitados</th><th>Estado</th></tr>${filtered.map((x) => `<tr><td>${x.event_date}</td><td>${x.client_name}</td><td>${x.guests}</td><td>${x.status}</td></tr>`).join("")}</table></body></html>`,
    );
  }
  return (
    <div className="moduleStack">
...
export function SettingsModule({
  ...
  async function addArea(e: React.FormEvent) {
    e.preventDefault();
    const r = await supabase.from("v2_areas").insert({
      restaurant_id: restaurantId,
      name: areaName.trim(),
