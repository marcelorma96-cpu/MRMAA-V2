            ...
            <div className="automationList">
              <section className="moduleCard"><h3>Áreas para reservaciones</h3>
                <form className="inlineForm" onSubmit={addArea}>
                  <input required value={areaName} onChange={e=>setAreaName(e.target.value)} placeholder="Nombre del área" />
                  <button className="primary">Agregar</button>
                </form>
                <div className="compactList">{areas.map(a=><article key={a.id}><b>{a.name}</b></article>)}</div>
                <small>Estas áreas también se utilizan en Horarios.</small>
              </section>
              <Toggle
