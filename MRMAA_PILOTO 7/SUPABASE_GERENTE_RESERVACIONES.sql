-- MRMAA: Gerente puede enviar reservaciones y cotizaciones de su restaurante a la papelera.
...
begin
  ...
  if entity in ('reservaciones','cotizaciones') then
    if rid is null or not public.v2_has_role(rid,array['administrador','admin','gerente']) then
      raise exception 'Solo el administrador o gerente puede enviar este registro a la papelera';
    end if;
  ...
  if entity='clientes' then
    ...
    if exists (select 1 from public.v2_reservations r where r.quote_id=target_id and r.restaurant_id=rid and r.deleted_at is null) then
