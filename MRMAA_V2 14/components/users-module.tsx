...
import { Power, RotateCcw, Trash2, UserPlus } from "lucide-react";
        ...
        {pageItems(filtered, page).map((x) => (
          <article key={x.user_id}>
            ...
            <div className="rowActions">
              ...
              {x.status !== "invitado" && <button title={x.status === "inactivo" ? "Activar acceso" : "Desactivar acceso"} onClick={() => toggleStatus(x.user_id)}><Power /> {x.status === "inactivo" ? "Activar" : "Desactivar"}</button>}
