...
export function ReservationsEnhanced({
  ...
  refreshToken?: number;
  preferences?: { reservation_show_people?: boolean; reservation_show_deposits?: boolean };
  rows?: Reservation[];
  ...
  const settings = { reservation_show_people: true, reservation_show_deposits: true, ...preferences };
  ...
  useListQuery({
    queryKey: restaurantId ? JSON.stringify([restaurantId, mode, date, from, to, querySearch, page]) : undefined,
    ...
    read: async signal => {
      ...
      const result = await applyFilters(supabase.from("v2_reservations").select("*", { count: "exact" })
        .eq("restaurant_id", restaurantId).is("deleted_at", null), querySearch)
        .order("event_date").order("event_time").range(first, first + 49).abortSignal(signal);
      if (result.error) throw result.error;
      return result;
    },
    onData: result => { setRows((result.data || []) as Reservation[]); setTotal(result.count || 0); setLoadError(""); },
  ...
  const showSummary = settings.reservation_show_people || settings.reservation_show_deposits;
  useListQuery({
    ...
    onData: metrics => { setPeople(Number(metrics?.people ?? 0)); setDeposits(Number(metrics?.deposits ?? 0)); setSummaryError(""); },
    ...
    try {
      ...
      if (warnings.length) {
        const examples = warnings.slice(0, 5).map((warning: any) =>
          `${warning.client_name || "Reserva"}: ${displayDate(warning.event_date)} ${String(warning.event_time || "").slice(0, 5)} · ${warning.area || "Sin área"}`,
        ).join("\n");
  ...
  async function print() {
    const outputRows = await allMatchingReservations();
    const printedPeople = outputRows.reduce((sum, row) => sum + Number(row.guests || 0), 0);
    ...
    const table =
      printMode !== "summary"
        ...
        : "";
    printHtml(
      `<html><head><title>Reservaciones</title><style>@page{size:A4 landscape;margin:8mm}body{font-family:Arial;color:#18181b}h1{font-family:Georgia}.metrics{display:flex;gap:10px;margin:14px 0}.metrics div{border:1px solid #ddd;padding:10px;min-width:110px}.metrics b,.metrics span,small{display:block}table{width:100%;table-layout:fixed;border-collapse:collapse}th,td{padding:5px 4px;border-bottom:1px solid #ddd;text-align:left;font-size:9px;vertical-align:top;overflow-wrap:anywhere}th{background:#18181b;color:white}small{color:#666;margin-top:3px}</style></head><body><h1>${escape(restaurantName)}</h1><p>Reporte de reservaciones · ${escape(new Date().toLocaleDateString(appLocale()))}</p>${metrics}${table}</body></html>`,
    );
