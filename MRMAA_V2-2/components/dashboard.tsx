...
export function Dashboard({
  ...
  const load = useCallback(async () => {
    ...
    setMemberRole(role);
    if (!["administrador", "admin"].includes(role) && ["reports", "settings"].includes(tab))
      setTab("reservations");
  ...
  useEffect(() => {
    ...
    const signOutAfterInactivity = () => {
      ...
      idleTimer = setTimeout(() => supabase.auth.signOut(), 60 * 60 * 1000);
    };
  ...
  const isAdmin = ["administrador", "admin"].includes(memberRole);
  ...
  async function moveManyToTrash(
    ...
    if (
      !window.confirm(
        `¿Enviar ${ids.length} registro(s) seleccionados a la papelera? Podrá restaurarlos durante 30 días.`,
      )
    )
      return;
    setError("");
      ...
      <div className="shell">
        <aside>
          ...
          <nav>
            ...
            {["administrador", "admin"].includes(memberRole) && (
              <>
                <Nav
                  active={tab === "reports"}
        ...
        <main className="workspace">
          ...
          ) : tab === "reports" && ["administrador", "admin"].includes(memberRole) ? (
            <EnhancedReports
              clients={clients}
              quotes={quotes}
              reservations={reservations}
            />
          ) : tab === "settings" && ["administrador", "admin"].includes(memberRole) ? (
            <EnhancedSettings
              restaurantId={restaurantId}
              restaurantName={restaurantName}
...
function Clients({
  ...
  function exportClients() {
    ...
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(data), "Clientes");
  ...
  return (
    <div className="moduleStack">
      <div className="listToolbar">
        ...
        {selected.length > 0 && <button className="dangerButton" onClick={async () => { await removeMany(selected); setSelected([]); }}><Trash2 /> Eliminar seleccionados ({selected.length})</button>}
        ...
        <div className="cards">
          {visible.map((c) => (
            <article key={c.id} className={selected.includes(c.id) ? "selectedRecord" : ""}>
              <input className="recordCheckbox" type="checkbox" aria-label={`Seleccionar ${c.name}`} checked={selected.includes(c.id)} onChange={() => setSelected((ids) => ids.includes(c.id) ? ids.filter((id) => id !== c.id) : [...ids, c.id])} />
              <div>
                <h3>{c.name}</h3>
                <p>
...
function Quotes({
  ...
  const [search, setSearch] = useState(""),
    [dateMode, setDateMode] = useState<"today" | "single" | "range" | "all">("all"),
    [date, setDate] = useState(currentDate()),
    [from, setFrom] = useState(currentDate()),
    ...
    filtered = rows.filter(
      (q) => {
        const matchesDate = dateMode === "all"
          ? true
          : dateMode === "today"
            ? q.event_date === currentDate()
            : dateMode === "single"
              ? q.event_date === date
              : q.event_date >= from && q.event_date <= to;
        const matchesSearch = !search.trim() ||
        [q.quote_number, q.client_name, q.client_phone, q.area, q.status, q.event_date, displayDate(q.event_date), q.event_time?.slice(0, 5)].some(
          (v) =>
            String(v || "")
              .toLowerCase()
              .includes(search.toLowerCase()),
    ...
    visible = pageItems(filtered, page);
  useEffect(() => setPage(1), [search, dateMode, date, from, to, rows.length]);
  useEffect(() => setSelected((ids) => ids.filter((id) => rows.some((row) => row.id === id))), [rows]);
  const visibleIds = visible.map((row) => row.id),
    allVisibleSelected = visibleIds.length > 0 && visibleIds.every((id) => selected.includes(id));
  return (
    <div className="moduleStack">
      <section className="reservationToolbar moduleCard quoteFilters">
        <div className="filterButtons">
          <button className={dateMode === "today" ? "active" : ""} onClick={() => setDateMode("today")}>Hoy</button>
          <button className={dateMode === "single" ? "active" : ""} onClick={() => setDateMode("single")}>Una fecha</button>
          <button className={dateMode === "range" ? "active" : ""} onClick={() => setDateMode("range")}>De–a</button>
          <button className={dateMode === "all" ? "active" : ""} onClick={() => setDateMode("all")}>Todas</button>
        </div>
        <div className="filterDates">
          <input className="moduleSearch" placeholder="Buscar cotización, cliente, fecha, hora, área o estado…" value={search} onChange={(e) => setSearch(e.target.value)} />
          {dateMode === "single" && <input type="date" value={date} onChange={(e) => setDate(e.target.value)} />}
          {dateMode === "range" && <><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} /><span>a</span><input type="date" value={to} onChange={(e) => setTo(e.target.value)} /></>}
        </div>
        <div className="listToolbar">
        {filtered.length > 0 && <label className="selectVisible"><input type="checkbox" checked={allVisibleSelected} onChange={() => setSelected(allVisibleSelected ? selected.filter((id) => !visibleIds.includes(id)) : Array.from(new Set([...selected, ...visibleIds])))} /> Seleccionar esta página</label>}
        {selected.length > 0 && <button className="dangerButton" onClick={async () => { await removeMany(selected); setSelected([]); }}><Trash2 /> Eliminar seleccionadas ({selected.length})</button>}
        </div>
      </section>
      {filtered.length ? (
        <div className="table">
          ...
          {visible.map((q) => (
            <div className={`tr quoteRow ${selected.includes(q.id) ? "selectedRecord" : ""}`} key={q.id}>
              <input className="recordCheckbox quoteCheckbox" type="checkbox" aria-label={`Seleccionar cotización ${q.quote_number}`} checked={selected.includes(q.id)} onChange={() => setSelected((ids) => ids.includes(q.id) ? ids.filter((id) => id !== q.id) : [...ids, q.id])} />
              <strong>#{q.quote_number}</strong>
              <span>{q.client_name}</span>
              <span>{displayDate(q.event_date)}</span>
              <span>{q.event_time?.slice(0, 5) || "—"}</span>
