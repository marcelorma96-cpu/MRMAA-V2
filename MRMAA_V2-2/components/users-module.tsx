...
export function UsersModule() {
  ...
  async function toggle(id: string) {
    try {
      const result = await request("PATCH", { user_id: id, action: "toggle" });
      setNotice(result.status === "activo" ? "Usuario activado." : "Usuario desactivado.");
      load();
    } catch (e: any) { setNotice(e.message); }
  }
        ...
        {pageItems(filtered, page).map((x) => (
          <article key={x.user_id}>
            ...
            <div className="rowActions">
              ...
              {x.status !== "invitado" && <button title={x.status === "inactivo" ? "Activar usuario" : "Desactivar usuario"} onClick={() => toggle(x.user_id)}>{x.status === "inactivo" ? "Activar" : "Desactivar"}</button>}
