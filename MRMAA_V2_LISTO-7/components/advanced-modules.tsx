...
export function MonthlySchedules({ restaurantId }: { restaurantId: string }) {
  ...
  function print() {
    const selected = rows
        .filter((x) => x.work_date >= printFrom && x.work_date <= printTo)
        .sort((a, b) =>
          `${areas.find((x) => x.id === a.area_id)?.name}-${employees.find((x) => x.id === a.employee_id)?.name}-${a.work_date}`.localeCompare(
            `${areas.find((x) => x.id === b.area_id)?.name}-${employees.find((x) => x.id === b.employee_id)?.name}-${b.work_date}`,
          ),
        ),
      win = window.open("", "_blank");
    if (!win) return;
    win.document.write(
      `<html><head><title>Horarios</title><style>body{font-family:Arial;padding:24px;color:#18181b}h1{font-family:Georgia}table{border-collapse:collapse;width:100%;font-size:10px}th,td{border:1px solid #bbb;padding:7px;text-align:left;vertical-align:top}th{background:#222;color:#fff}.employee{font-weight:bold}.off{background:#f4f4f5}.permission{background:#fff7ed}@media print{body{padding:0}}</style></head><body><h1>Horario de empleados</h1><p>Periodo: ${esc(printFrom)} a ${esc(printTo)}</p><table><tr><th>Área</th><th>Empleado</th><th>Día y fecha</th><th>Horario / asignación</th><th>Tiempo de comida</th><th>Indicaciones</th></tr>${selected
        .map((x) => {
          const shift = shifts.find((s) => s.id === x.shift_id),
            day = new Date(`${x.work_date}T12:00:00`).toLocaleDateString(
              "es-GT",
              {
                weekday: "long",
                day: "2-digit",
            ...
            assignment =
              x.entry_type === "work"
                ? `${shift?.name || "Turno"}: ${shift?.start_time?.slice(0, 5) || "—"} a ${shift?.end_time?.slice(0, 5) || "—"}`
                : x.entry_type === "rest"
                  ? "Día de descanso"
                  : "Permiso autorizado";
          return `<tr class="${x.entry_type === "rest" ? "off" : x.entry_type === "permission" ? "permission" : ""}"><td>${esc(areas.find((a) => a.id === x.area_id)?.name || "Sin área")}</td><td class="employee">${esc(employees.find((e) => e.id === x.employee_id)?.name)}</td><td>${esc(day)}</td><td>${esc(assignment)}</td><td>${x.entry_type === "work" && x.break_start && x.break_end ? `${esc(x.break_start.slice(0, 5))} a ${esc(x.break_end.slice(0, 5))}` : "—"}</td><td>${esc(x.notes || "Sin indicaciones adicionales")}</td></tr>`;
        })
