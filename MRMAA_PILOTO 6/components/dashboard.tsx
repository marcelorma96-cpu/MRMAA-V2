...
export function Dashboard({
  ...
  const searchQuoteProduct = (index: number, name: string) => {
    const product = quoteProducts.find((x) => x.name.toLowerCase() === name.trim().toLowerCase());
    setQuoteForm((f: any) => {
      return {
        ...f,
        items: f.items.map((x: QuoteItem, i: number) => i !== index ? x : {
          ...x,
          name,
          ...(product ? { name: product.name, description: product.description || "", unit_price: Number(product.price) } : {}),
        }),
      };
    });
  };
            ...
            <div className="items">
              <datalist id="quote-product-suggestions">
                {quoteProducts.map((product) => (
                  <option key={product.id} value={product.name} label={`${product.description || ""} · ${money(Number(product.price))}`} />
                ))}
              </datalist>
              ...
              {quoteForm.items.map((x: QuoteItem, i: number) => (
                <div className="itemRow" key={i}>
                  <input
                    ...
                    aria-label={`Producto o servicio ${i + 1}`}
                    value={x.name}
                    onChange={(e) => searchQuoteProduct(i, e.target.value)}
                  />
                  <textarea
                    rows={3}
                  ...
                  <strong>
                    ...
                    <button
                      type="button"
                      className="quoteLineDelete"
                      title="Eliminar línea"
                      aria-label={`Eliminar línea ${i + 1}`}
                      onClick={() => field("items", quoteForm.items.filter((_: QuoteItem, index: number) => index !== i))}
                    >
                      <Trash2 size={16} />
                    </button>
                  </strong>
