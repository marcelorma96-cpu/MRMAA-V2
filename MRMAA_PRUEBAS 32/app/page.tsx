  ...
  return <main className="landing" translate="no">
    ...
    <nav className="landingNav"><a className="landingLogo" href="#top"><span className="mark">M</span><strong>MRMAA</strong></a>
      ...
      <div className="landingAuthActions">
        ...
        {PUBLIC_SIGNUP_ENABLED && <button type="button" className="primary" onClick={() => startTrial("advanced", interval)}>{copy("Registrarse", "Sign up")}</button>}
      </div></div></nav>
    <section className="hero" id="top"><div><span className="heroTag">{copy("Administración para restaurantes", "Restaurant management")}</span>
      <h1>{copy("Reservas y cotizaciones", "Reservations and quotes")}<br/><em>{copy("en un solo lugar.", "in one place.")}</em></h1>
      <p>{copy("Organice las reservas de su restaurante, con o sin cotización. Controle fechas, invitados y anticipos; mantenga conectados a sus clientes y los horarios de su equipo.", "Organize restaurant reservations with or without a quote. Track dates, guests and deposits, and keep customer records and team schedules together.")}</p>
      <div className="heroActions"><button className="primary" onClick={() => PUBLIC_SIGNUP_ENABLED ? startTrial("advanced") : login()}>{copy(PUBLIC_SIGNUP_ENABLED ? "Pruebe gratis 10 días" : "Iniciar sesión", PUBLIC_SIGNUP_ENABLED ? "Try free for 10 days" : "Sign in")}<ArrowRight/></button><a className="landingSecondary" href="#demostracion">{copy("Ver cómo funciona", "See how it works")}</a></div><p className="trialReassurance">{copy("10 días gratis · Sin tarjeta · Desde su navegador", "10 days free · No card required · In your browser")}</p></div>
    ...
    <section className="landingDemo" id="demostracion"><div><span className="heroTag">{copy("Vea MRMAA por dentro", "See inside MRMAA")}</span><h2>{copy("De la cotización a la reserva.", "From quote to reservation.")}</h2><p>{copy("Una demostración con pantallas reales. También puede crear una reserva directamente, sin preparar una cotización.", "A demonstration with real screens. You can also create a reservation directly, without preparing a quote.")}</p><button type="button" className="primary" onClick={() => PUBLIC_SIGNUP_ENABLED ? startTrial("advanced") : login()}>{copy("Pruebe gratis 10 días", "Try free for 10 days")}<ArrowRight/></button><p className="trialReassurance">{copy("Sin tarjeta. Soporte por email.", "No card required. Email support.")}</p></div><figure><video controls playsInline preload="none" poster="/mrmaa-demo-poster.jpg" aria-label={copy("Demostración de MRMAA en español, sin audio", "MRMAA demonstration in Spanish, without audio")}><source src="/mrmaa-demo.mp4" type="video/mp4"/></video><figcaption>{copy("Pantallas reales · Datos de demostración · Sin audio", "Real screens · Demo data · Spanish text · No audio")}</figcaption></figure></section>
