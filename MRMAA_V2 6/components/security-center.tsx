...
type MemberIdentity = { user_id: string; name: string; email: string };
...
export function SecurityCenter({ restaurantId, exportOnly = false }: { restaurantId: string; exportOnly?: boolean }) {
  const [deleted, setDeleted] = useState<DeletedRow[]>([]),
    ...
    [identities, setIdentities] = useState<Record<string, string>>({}),
  ...
  const load = useCallback(async () => {
    ...
    const [member, clients, quotes, reservations, history, members] = await Promise.all([
      ...
      supabase
        .from("v2_audit_log")
        .select("id,table_name,action,changed_by,actor_name,changed_at,old_data,new_data")
        .eq("restaurant_id", restaurantId)
        .order("changed_at", { ascending: false })
        .limit(200),
      supabase.from("v2_members").select("user_id,name,email").eq("restaurant_id", restaurantId),
    ]);
    ...
    setIdentities(Object.fromEntries(((members.data || []) as MemberIdentity[]).map((x) => [x.user_id, x.name || x.email || x.user_id])));
    const error =
      ...
      <section className="moduleCard">
        <div className="moduleTitle">
          <div>
            <h2>Papelera</h2>
            <p>
              Los registros pueden eliminarse definitivamente después de 30
              días.
            </p>
          </div>
        ...
        <div className="auditTable">
          {pageItems(auditRows, auditPage).map((row) => (
            <article key={row.id}>
              ...
              <small>{row.actor_name || identities[row.changed_by || ""] || (row.changed_by ? "Usuario anterior" : "Sistema")} · {new Date(row.changed_at).toLocaleString("es-GT")}</small>
            </article>
          ))}
          {!auditRows.length && (
            <div className="empty">
