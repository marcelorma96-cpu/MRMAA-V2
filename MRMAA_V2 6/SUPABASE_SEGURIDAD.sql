...
begin
  ...
  insert into public.v2_audit_log(restaurant_id,table_name,record_id,action,old_data,new_data,changed_by,actor_name)
  values(rid,tg_table_name,rec_id,rid_action,before_row,after_row,auth.uid(),coalesce(current_actor_name,'Sistema'));
  return case when tg_op='DELETE' then old else new end;
end $$;
