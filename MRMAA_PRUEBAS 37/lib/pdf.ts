import { quotePageEnd, quoteSourceRanges, sizeQuoteColumns } from "./pdf-pagination";
...
import { needsNativePdfViewer, openQuotePrintDocument } from "./print";
import { createQuoteDocument, type QuotePageImage } from "./quote-document";

const money = (n: number, currency: string) =>
  formatAppMoney(n, appCurrency(currency));

export function quotePdfFilename(quoteNumber: number, language = currentAppLanguage()) {
  const safeNumber = String(quoteNumber).replace(/[^0-9A-Za-z_-]/g, "");
  return `${language === "en" ? "Quote" : "Cotizacion"}-${safeNumber || "0"}.pdf`;
}
...
export async function captureQuotePreviewPages(element: HTMLElement): Promise<QuotePageImage[]> {
  ...
  const mobileCapture = needsNativePdfViewer()
    || (typeof window !== "undefined" && window.innerWidth <= 600);
  const canvas = await html2canvas(element, {
    // A smaller mobile scale avoids Safari's canvas-memory limit while still
    // producing a sharp document (about 160 dpi at A4 width).
    scale: mobileCapture ? 1.5 : 2,
    width: 900,
    windowWidth: 1100,
    ...
    onclone: (_document, paper) => {
      ...
      const viewport = paper.closest<HTMLElement>(".quotePreviewViewport");
      if (viewport) { viewport.style.height = "auto"; viewport.style.overflow = "visible"; viewport.style.width = "900px"; }
      sizeQuoteColumns(paper, _document);
  ...
  if (imageHeight <= pageHeight * 1.18) {
    const fittedWidth = Math.min(
      ...
      x = (210 - fittedWidth) / 2;
    pages.push({ image: canvas.toDataURL(mobileCapture ? "image/jpeg" : "image/png", 0.96), x, y: margin, width: fittedWidth, height: fittedHeight });
  } else {
    const sourcePageHeight = Math.floor(canvas.width * pageHeight / pageWidth);
    const scale = canvas.width / capturedCssWidth;
    const ranges = quoteSourceRanges(protectedRanges, scale);
    let top = 0;
    while (top < canvas.height) {
      ...
      pages.push({ image: pageCanvas.toDataURL(mobileCapture ? "image/jpeg" : "image/png", 0.96), x: margin, y: margin, width: pageWidth, height: (sliceHeight * pageWidth) / canvas.width });
...
export async function downloadQuotePreviewPdf(
  ...
  const filename = quotePdfFilename(quoteNumber);
  const title = filename.slice(0, -4);
  const pdf = createQuoteDocument(pages, title);
  if (target || useViewer) return { ...openQuotePrintDocument(pdf.output("blob"), target), pages: pages.length };
  pdf.save(filename);
  return { url: "", opened: false, pages: pages.length };
}

export async function printQuotePreview(element: HTMLElement, quoteNumber: number, target: Window | null) {
  const pages = await captureQuotePreviewPages(element);
  const title = quotePdfFilename(quoteNumber).slice(0, -4);
  const pdf = createQuoteDocument(pages, title, true);
  return { ...openQuotePrintDocument(pdf.output("blob"), target), pages: pages.length };
}
...
export function downloadQuotePdf(
  ...
  pdf.text(restaurantName || "UnoMesa", settings.logo_data_url ? 43 : 14, 17);
  pdf.setFontSize(10);
  ...
  pdf.text(formatEventTime(quote.event_time, settings.time_format), 157, 63);
  pdf.setFontSize(8);
  pdf.setTextColor(100);
  ...
  if (settings.quote_show_deposit_view !== false) {
    pdf.text(t("Anticipo"), labelX, y);
    pdf.text(`- ${money(totals.deposit, currency)}`, valueX, y, { align: "right" });
    y += 9;
  }
  pdf.setFont(font, "bold");
  pdf.setFontSize(12);
  ...
  pdf.save(quotePdfFilename(quote.quote_number));
}
