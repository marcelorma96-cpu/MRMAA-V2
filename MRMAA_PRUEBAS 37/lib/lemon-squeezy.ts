...
export function lemonURL(value: unknown) {
  ...
  // Lemon uses our configured custom domain for checkout and customer-portal
  // URLs too. Trust this exact host, never arbitrary UnoMesa subdomains or a
  // hostname supplied by the browser. Keep the original Lemon URLs valid.
  const allowedHost = ["pay.mrmaa.com", "pay.unomesa.com"].includes(url.hostname)
    || /^(?:[a-z0-9-]+\.)?lemonsqueezy\.com$/.test(url.hostname);
  if (url.protocol !== "https:" || url.username || url.password || url.port
    || !allowedHost) throw new Error("Invalid payment URL");
  return url.href;
}
...
export function verifyLemonEvent(raw: string, signature: string | null, secret = process.env.LEMON_SQUEEZY_WEBHOOK_SECRET || "") {
  if (!secret) throw new BillingError("Los pagos todavía no están configurados.", 503, "WEBHOOK_SECRET_MISSING");
  if (!signature || !/^[0-9a-f]{64}$/i.test(signature)) throw new BillingError("Firma inválida.", 400, "WEBHOOK_SIGNATURE_INVALID");
  const expected = createHmac("sha256", secret).update(raw).digest();
  if (!timingSafeEqual(expected, Buffer.from(signature, "hex"))) throw new BillingError("Firma inválida.", 400, "WEBHOOK_SIGNATURE_INVALID");
  let event: any;
  try { event = JSON.parse(raw); } catch { throw new BillingError("Evento no permitido.", 400); }
  if (!event?.meta?.event_name) throw new BillingError("Evento no permitido.", 400, "WEBHOOK_EVENT_INVALID");
  if (event.data?.attributes?.test_mode !== LEMON_TEST_MODE) throw new BillingError("Evento no permitido.", 400, "WEBHOOK_MODE_MISMATCH");
