...
/** Preserve the verified legacy sender until the new address is configured. */
export function mailSender(kind: "passcode" | "support") {
  const configured = kind === "passcode" ? process.env.UNOMESA_PASSCODE_FROM : process.env.UNOMESA_SUPPORT_FROM;
  return configured?.trim() || process.env.MRMAA_SMTP_FROM;
}
