...
export async function POST(req: NextRequest) {
  try {
    ...
    const intent = await admin.rpc("v2_begin_registration", { p_data: details });
    if (intent.error) throw new BillingError(intent.error.code === "P0001" ? intent.error.message : "No se pudo crear la cuenta. Revise sus datos e intente nuevamente.");
    ...
    return NextResponse.json({ ok: true, session: result.data.session }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    return NextResponse.json({ error: error instanceof BillingError ? error.message : "No se pudo crear la cuenta. Intente nuevamente." }, { status: error instanceof BillingError ? error.status : 503 });
  }
}
