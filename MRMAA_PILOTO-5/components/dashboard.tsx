...
import { needsNativePdfViewer, printHtml, reserveQuotePrintWindow } from "@/lib/print";
...
export function Dashboard({
  ...
  const tutorialSteps = useMemo(() => {
    const steps: { tab?: Tab; title: string; text: string }[] = [
      ...
      { tab: "schedules", title: "Horarios", text: "Para imprimir, elija Corrido o Página por área y Escala de impresión. Menor escala reorganiza el contenido hacia arriba antes de generar el PDF. Organice empleados por área, asigne horarios y comidas, copie patrones y trabaje con varios empleados. Puede minimizar el cuadro con el botón superior. En computadora, arrastre su esquina inferior derecha para cambiar la altura; el aviso está junto a esa esquina. Excel e Imprimir utilizan el rango de fechas seleccionado." },
    ];
  ...
  async function moveToTrash(
    ...
    if (!(await authorize(entity === "reservaciones" ? "canDeleteReservations" : entity === "cotizaciones" ? "canDeleteQuotes" : "isAdmin"))) return;
  ...
  async function moveManyToTrash(
    ...
    if (!ids.length || !(await authorize(entity === "reservaciones" ? "canDeleteReservations" : entity === "cotizaciones" ? "canDeleteQuotes" : "isAdmin"))) return;
        ...
        >
          ...
          <form onSubmit={saveQuote} className="quoteForm">
            <div className="grid3">
              ...
              <Field label="Área">
                <AreaPicker searchOnly areas={areas} value={quoteForm.area} areaId={quoteForm.area_id}
  ...
  return (
    <div className="overlay">
      ...
      <div
        ...
        className="quotePreviewViewport"
        style={{ height: scaledPaperHeight ? `${scaledPaperHeight}px` : undefined }}
      >
      <section
        ref={paperRef}
      ...
      >
        ...
        <footer className="quotePreviewActions">
          {outputError && <p role="alert">{outputError}</p>}
          {output && <span role="status">Preparando documento…</span>}
          {!output && printReady && fallbackPurpose === "print" && <p className="quotePrintHelp" role="status">Use el botón de impresión del PDF si el diálogo no se abre automáticamente. Seleccione Ajustar al área imprimible.</p>}
