...
export async function downloadQuotePreviewPdf(
  ...
  if (imageHeight <= pageHeight * 1.18) {
    ...
    const sourcePageHeight = Math.floor(
      ...
      scaleY = canvas.height / element.scrollHeight,
      elementTop = element.getBoundingClientRect().top,
      safeBreaks = Array.from(
        element.querySelectorAll("header, .quoteInfo, .previewTable > div, .previewTotals > div, .note"),
      )
        .map((node) => Math.round((((node as HTMLElement).getBoundingClientRect().bottom - elementTop) * scaleY)))
        .filter((value) => value > 0 && value < canvas.height)
        .sort((a, b) => a - b);
    let top = 0, page = 0;
    while (top < canvas.height) {
      const target = Math.min(top + sourcePageHeight, canvas.height);
      const candidates = safeBreaks.filter((point) => point > top + sourcePageHeight * 0.45 && point <= target - 8 * scaleY);
      const bottom = target === canvas.height ? target : (candidates.at(-1) || target);
