...
export function Dashboard({
  ...
  useEffect(() => {
    if (!preview || previewExporting || !recordVersion) return;
    let cancelled = false;
    const id = preview.id;
    void (async () => {
      const [q, items] = await Promise.all([
        supabase.from('v2_quotes').select('*').eq('restaurant_id', restaurantId).eq('id', id).is('deleted_at', null).maybeSingle(),
        supabase.from('v2_quote_items').select('id,name,description,quantity,unit_price').eq('quote_id', id).order('position'),
      ]);
      if (cancelled) return;
      if (q.error || items.error) { setError(LOAD_RETRY_MESSAGE); return; }
      if (!q.data) { setPreview(null); return; }
      setPreview(current => current?.id === id ? { ...q.data, items: items.data || [] } as Quote : current);
    })().catch(() => { if (!cancelled) setError(LOAD_RETRY_MESSAGE); });
...
export function Clients({
  ...
  }, [restaurantId, normalizedSearch, page, refreshToken, remoteVersion]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const links = ids.length
        ...
        : { data: [], error: null };
      if (links.error) {
        if (!cancelled) { setLoadError(links.error.message); setLoading(false); }
        return;
      }
