...
export function Dashboard({
  ...
  const { isAdmin, canRead, canOperate, canManageSchedules, canManageQuoteProducts, canManageReservationAreas } = permissionsFor(memberRole, memberStatus);
  useEffect(() => {
    if (!canOperate) {
      setClientOpen(false);
      setQuoteOpen(false);
            ...
            <div className="helpContent">
              <section>
                ...
                <div className="permissionTable">
                  ...
                  <div><strong>Gerente</strong><span>Crear y modificar reservaciones, clientes, cotizaciones, horarios, menús y productos. Agregar, editar y eliminar áreas de reservaciones. Sin reportes, usuarios ni configuración administrativa.</span></div>
              ...
              <HelpSection title="Cotizaciones" text="Nuevo abre el formulario. Ingrese cliente, evento, productos, descuentos, propina, anticipo y método de pago. Ver abre el preview sin cambiar de sección; PDF descarga el documento completo; Imprimir utiliza las mismas páginas y márgenes del PDF; Editar modifica; Convertir en reserva transfiere la información y marca ambos registros como confirmados. Para eliminar una cotización convertida, elimine primero la reservación vinculada." />
...
export function QuotePreview({
  ...
  async function exportDocument(action: "pdf" | "print") {
    ...
    setOutput(action); setOutputError("");
    try {
      ...
      if (action === "pdf") await downloadQuotePreviewPdf(paperRef.current, quote.quote_number);
        ...
        const pages = await printQuotePreview(paperRef.current, quote.quote_number);
        void recordAuditActivity(restaurantId, "impresion", "cotizaciones", { quote_id: quote.id, quote_number: quote.quote_number, pages });
      }
    } catch {
      setOutputError(userMessage(null));
    } finally { outputBusy.current = false; setOutput(null); }
  }
  const t = calculateQuoteWithAdjustments(
    quote.items,
    quote.discount_pct,
