...
await measured("records_insert_ms", `
  ...
  select n,1+((n-1)%${restaurants}),date '2026-01-01'+((n-1)%730)::integer,
...
const output = {
  ...
  passed: Number(isolation.rows[0].tenants) === restaurants && Number(isolation.rows[0].records) === records && queries.every((query) => query.rows <= 50),
};
