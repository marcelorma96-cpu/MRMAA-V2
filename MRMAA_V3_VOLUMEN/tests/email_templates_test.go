...
func TestSupabaseEmailTemplates(t *testing.T) {
  ...
  cases := []struct { name string; data any; lang string }{
    ...
    {"fr_fallback", map[string]any{"language":"fr"}, "es"},
  ...
  expectedConfirm := map[string]string{"es":"Confirme su correo", "en":"Confirm your email"}
  expectedInvite := map[string]string{"es":"Invitación de acceso", "en":"Account invitation"}
  expectedReset := map[string]string{"es":"Restablecer contraseña", "en":"Reset your password"}
