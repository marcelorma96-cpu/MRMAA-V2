...
async function context(req: NextRequest, restaurantId: string) {
  ...
  const publicClient = createClient(url, anon, { auth: { persistSession: false }, global: { headers: { Authorization: `Bearer ${token}` } } });
  ...
  await requireVerifiedMfa(authResult.data.user, token);
  const membership = await publicClient.rpc("v2_effective_membership", { p_restaurant: restaurantId });
  ...
  const access = await publicClient.rpc("v2_billing_can_write", { p_restaurant: restaurantId });
  if (access.error || !access.data) throw new Error("Su prueba o suscripción terminó. Active un plan para realizar cambios.");
  return { admin, client: publicClient };
}

export async function POST(req: NextRequest) {
  try {
    requestSafety(req);
    const payload = await readBoundedJson(req, 10_000);
    ...
    const { admin, client } = await context(req, restaurantId);
    await takeDistributedRateLimit(admin, req, "products:write", 30);
    const result = productId
      ? await client.from("v2_quote_products").update({ name, description, price })
          .eq("id", productId).eq("restaurant_id", restaurantId).select("*").maybeSingle()
      : await client.from("v2_quote_products").insert({ restaurant_id: restaurantId, name, description, price })
          .select("*").single();
    if (result.error) {
