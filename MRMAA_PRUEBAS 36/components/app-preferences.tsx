...
import { useRef, createContext, startTransition, useCallback, useContext, useEffect, useMemo, useState } from "react";
...
export function confirmApp(message: string) {
  return window.confirm(translate(message, activeLanguage));
}
...
function translateElement(root: ParentNode, language: AppLanguage) {
  ...
  while (node) {
    ...
    if (parent && !parent.closest("script,style,textarea,code,pre,[contenteditable='true'],[translate='no']")) {
...
export function AppPreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    let saved: string | null = null, savedCurrency: string | null = null;
    try { saved = localStorage.getItem(LANGUAGE_KEY); savedCurrency = localStorage.getItem(CURRENCY_KEY); } catch {}
    const detected = (navigator.languages?.[0] || navigator.language || "en").toLowerCase().startsWith("es") ? "es" : "en";
    const initial = saved === "es" || saved === "en" ? saved : detected;
    activeLanguage = initial; activeCurrency = appCurrency(savedCurrency);
    setLanguage(initial); setCurrency(activeCurrency);
    if (saved !== "es" && saved !== "en") {
      void fetch("/api/locale", { signal: controller.signal, cache: "no-store" })
        .then(response => response.ok ? response.json() : null).then(result => {
          if (controller.signal.aborted || !["es", "en"].includes(result?.language)) return;
          // A manual selection or restaurant preference made while loading wins.
          try { if (localStorage.getItem(LANGUAGE_KEY)) return; } catch {}
          if (preferenceChanged.current) return;
          activeLanguage = result.language; setLanguage(result.language);
        }).catch(() => {});
    }
  ...
  const setPreferences = useCallback((nextLanguage: unknown, nextCurrency: unknown) => {
    ...
    try { localStorage.setItem(LANGUAGE_KEY, safeLanguage); localStorage.setItem(CURRENCY_KEY, safeCurrency); } catch {}
