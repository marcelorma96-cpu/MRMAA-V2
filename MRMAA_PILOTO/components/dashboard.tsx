...
import { readQuoteDetail, readQuotePage, saveQuote as writeQuote, type QuoteSavePayload, type QuoteSummary } from "@/lib/quote-data";
import { useListSearch } from "@/lib/list-search";
import { useListQuery } from "@/lib/list-query";
import { money, displayDate, Nav, Field, Actions, Total, HelpSection, Modal } from "@/components/dashboard-ui";
import { ClientSearchSelect } from "@/components/clients-panel";
...
export function Dashboard({
  ...
  useEffect(() => { if (!planFor(planCode).schedules && ["schedules", "reports"].includes(tab)) setTab("reservations"); }, [planCode, tab]);
  const navigateTab = (next: Tab) => {
    if (next === tab || (["schedules", "reports"].includes(next) && !planFor(planCode).schedules)) return;
  ...
  useEffect(() => {
    ...
    const back = (event: PopStateEvent) => {
      ...
      if (browserNavigation.current.previewExporting || !confirmDiscardChanges()) {
  ...
  const tutorialSteps = useMemo(() => {
    ...
    if (isAdmin) {
      steps.push(
        ...
        { tab: "settings", title: "Planes y suscripción", text: "Abra Configuración → Suscripción para consultar su plan, fecha de vencimiento y opciones de pago. Solo el administrador principal gestiona los pagos. La prueba dura 5 días sin tarjeta; el pago anual equivale a 10 mensualidades." },
      );
    }
    ...
    return planFor(planCode).schedules ? steps : steps.filter(step => step.tab !== "schedules" && step.tab !== "reports");
  }, [isAdmin, canManageQuoteProducts, canManageReservationAreas, planCode]);
  useEffect(() => {
  ...
  async function quotesCanBeDeleted(ids: string[]) {
    ...
    if (result.error) { setError(userMessage(result.error)); return false; }
  ...
  async function moveToTrash(
    ...
    if (error) return setError(userMessage(error));
    setPreview(null);
    setDataVersion((value) => value + 1);
  }
  async function moveManyToTrash(
    ...
    if (failed) return setError(userMessage(failed));
    setPreview(null);
    setDataVersion((value) => value + 1);
  }
  ...
  async function saveQuote(e: React.FormEvent) {
    ...
    try {
      await writeQuote(supabase, restaurantId, quoteEdit?.id || null, payload, valid);
    } catch (err: any) {
      ...
      <div className="shell dashboardRefined">
        <aside>
          ...
          <nav>
            ...
            {isAdmin && planFor(planCode).schedules && (
              <>
                <Nav
                  active={tab === "reports"}
          ...
          <a
            ...
            href={`mailto:support@mrmaa.com?subject=${encodeURIComponent(`${language === "en" ? "MRMAA support request" : "Solicitud de soporte MRMAA"} - ${restaurantName}`)}`}
          >
            <Mail />
            <span>Soporte</span>
          </a>
          <a
            className="tutorialLauncher"
            href={`mailto:suggestions@mrmaa.com?subject=${encodeURIComponent(`${language === "en" ? "Suggestion for MRMAA" : "Sugerencia para MRMAA"} - ${restaurantName}`)}`}
          >
            <MessageSquare />
        ...
        <main className="workspace">
          ...
          ) : tab === "reports" && isAdmin && planFor(planCode).schedules ? (
            <EnhancedReports
              restaurantId={restaurantId}
            />
        ...
        <div className="modalBackdrop helpBackdrop" onMouseDown={(event) => event.target === event.currentTarget && setHelpOpen(false)}>
          <section className="helpCenter" role="dialog" aria-modal="true" aria-label="Instructivo de MRMAA">
            ...
            <div className="helpContent">
              ...
              {isAdmin && <HelpSection title="Planes y suscripción" text="Abra Configuración → Suscripción para consultar su plan, fecha de vencimiento y opciones de pago. Solo el administrador principal gestiona los pagos. La prueba dura 5 días sin tarjeta; el pago anual equivale a 10 mensualidades." />}
        ...
        >
          ...
          <form onSubmit={saveQuote} className="quoteForm">
            <div className="grid3">
              ...
              <Field label="Área">
                <AreaPicker required areas={areas} value={quoteForm.area} areaId={quoteForm.area_id}
