...
export function PlanCards({ interval, onInterval, onChoose, busy = false, current, currentInterval, signup = false, payNow = false, minimumUsers = 0 }: {
  interval: BillingInterval; onInterval: (value: BillingInterval) => void;
  onChoose: (plan: PlanCode) => void; busy?: boolean; current?: string; currentInterval?: BillingInterval; signup?: boolean; payNow?: boolean; minimumUsers?: number;
}) {
  ...
  return <div translate="no" className="billingPlans">
    ...
    <div className="planGrid">{PLANS.map(plan => {
      ...
      <button type="button" className="primary" onClick={() => onChoose(plan.code)} disabled={busy || plan.users < minimumUsers || isCurrent}>
        {isCurrent ? (en ? "Current plan" : "Plan actual") : plan.users < minimumUsers ? (en ? "Contact support to change" : "Contacte a soporte para cambiar") : busy ? (en ? "Processing…" : "Procesando…") : payNow ? (en ? "Pay now" : "Pagar ahora") : signup ? (en ? `Try ${plan.name}` : `Probar ${plan.name}`) : isUpgrade ? (en ? `Upgrade to ${plan.name}` : `Mejorar a ${plan.name}`) : current === plan.code ? (en ? `Switch to ${interval === "year" ? "annual" : "monthly"}` : `Cambiar a pago ${interval === "year" ? "anual" : "mensual"}`) : (en ? `Choose ${plan.name}` : `Elegir ${plan.name}`)}
      </button>
