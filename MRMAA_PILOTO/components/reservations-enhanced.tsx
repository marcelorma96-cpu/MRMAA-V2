...
export function ReservationsEnhanced({
  ...
  refreshToken?: number;
  preferences?: { reservation_show_people?: boolean; reservation_show_deposits?: boolean; reservation_show_deposits_list?: boolean };
  rows?: Reservation[];
  ...
  async function print() {
    ...
    const table =
      printMode !== "summary"
        ? `<table><thead><tr><th>Fecha</th><th>Hora</th><th>Cliente</th><th>Área</th><th>Inv.</th><th>Menú</th>${settings.reservation_show_deposits_list ? '<th>Anticipo</th><th>Método</th>' : ''}<th>Estado</th><th>Observaciones</th></tr></thead><tbody>${outputRows.map((r) => `<tr><td>${escape(displayDate(r.event_date))}</td><td>${escape(r.event_time?.slice(0, 5))}</td><td>${escape(r.client_name)}<small>${escape(r.phone)}</small></td><td>${escape(r.area)}</td><td>${r.guests ?? ""}</td><td>${escape(r.menu)}</td>${settings.reservation_show_deposits_list ? `<td>${r.deposit == null ? "" : escape(money(Number(r.deposit)))}</td><td>${escape(paymentMethodLabel(r.payment_method))}</td>` : ''}<td>${escape(r.status)}</td><td>${escape(r.notes)}</td></tr>`).join("")}</tbody></table>`
        : "";
    printHtml(
