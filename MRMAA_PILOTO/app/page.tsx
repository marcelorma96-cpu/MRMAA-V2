...
export default function Home() {
  ...
  async function loadAccount() {
    ...
    setMessage("");
    try {
      const verifiedAccount = await readVerifiedAccount(supabase, session?.user.id || "");
  ...
  useEffect(() => {
    ...
    let lastRefresh = 0;
    const refresh = () => { if (document.visibilityState === "visible" && Date.now() - lastRefresh > 15000) { lastRefresh = Date.now(); void loadAccount(); } };
    ...
    const timer = !account.trialExempt && account.canWrite && deadlines.length ? setTimeout(refresh, delay) : undefined;
    return () => { window.removeEventListener("mrmaa:billing-updated", paymentRefresh); window.removeEventListener("focus", refresh); if (timer) clearTimeout(timer); };
...
function TrialBanner({ endsAt, canManage, onManage }: { endsAt: string; canManage: boolean; onManage: () => void }) {
  ...
  const hours = Math.max(0, Math.ceil((new Date(endsAt).getTime() - now) / 3600000));
  return <div className="trialBanner" translate="no"><span>{en ? `Free trial: ${Math.floor(hours / 24)} days and ${hours % 24} hours remaining.` : `Prueba gratuita: quedan ${Math.floor(hours / 24)} días y ${hours % 24} horas.`}</span>{canManage && <button type="button" onClick={onManage}>{en ? "View plans" : "Ver planes"}</button>}</div>;
}
...
function Landing({
  ...
  startTrial: (p: PlanCode, interval?: BillingInterval) => void;
  login: () => void;
}) {
    ...
    <main className="landing">
      ...
      <section className="hero" id="top">
        <div>
          ...
          <div className="heroActions">
            ...
            <span>Prueba gratis de 5 días, sin tarjeta.</span>
          </div>
        </div>
      ...
      <section className="plansSection" id="planes">
        <div className="sectionIntro">
          ...
          <p>Pruebe durante 5 días. No solicitaremos información de pago.</p>
        </div>
        <PlanCards interval={interval} onInterval={setInterval} onChoose={plan => PUBLIC_SIGNUP_ENABLED && startTrial(plan, interval)} signup busy={!PUBLIC_SIGNUP_ENABLED} />
      </section>
      <MobileComingSoon />
...
function AccessForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (currentMode === "login") {
      ...
      if (!response.ok) setFeedback(result.error || "No se pudo crear la cuenta. Intente nuevamente.");
        ...
        setFeedback("¡Cuenta creada con éxito! Revise su correo y confirme la cuenta. Después podrá iniciar sesión.");
      }
    }
    } catch {
        ...
        <div>
          ...
          <p>
            {showRecovery
              ...
              : currentMode === "signup"
              ? `Plan ${plan.name}: 5 días gratis, sin tarjeta.`
