...
export async function webhook(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "No se pudo procesar el evento." }, { status: error instanceof RequestSafetyError ? error.status : error instanceof BillingError && error.status === 400 ? 400 : 503 });
  } finally { if (locked) await releaseBilling(locked.admin, locked.id, locked.token); }
}
