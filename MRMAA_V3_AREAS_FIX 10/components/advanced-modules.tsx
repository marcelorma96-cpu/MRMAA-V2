...
import { CalendarDays, Copy, Maximize2, Minimize2, Pencil, Printer, Sparkles, Trash2, X } from "lucide-react";
...
import { appCurrency, appLanguage, appLocale, formatAppMoney, useAppPreferences } from "@/components/app-preferences";
...
export function EnhancedSettings({
  ...
  async function addProduct(e: React.FormEvent) {
    ...
    try {
      ...
      const r = productEditId
        ? await supabase.from("v2_quote_products").update(payload).eq("id", productEditId).eq("restaurant_id", restaurantId)
        : await supabase.from("v2_quote_products").insert({ restaurant_id: restaurantId, ...payload });
      if (r.error) {
        if (r.error.code !== "23505" || productEditId) {
          setNotice(r.error.code === "23505" ? "Ya existe un menú o producto con ese nombre. Use Editar en el listado para modificarlo." : r.error.message);
          return;
        }
        // A retry after a completed insert must not overwrite an existing menu.
        const existing = await supabase.from("v2_quote_products")
          .select("id,description,price").eq("restaurant_id", restaurantId).eq("name", name).maybeSingle();
        if (existing.error) { setNotice(existing.error.message); return; }
        if (!existing.data || (existing.data.description || "") !== (payload.description || "") || Number(existing.data.price) !== payload.price) {
          setNotice("Ya existe un menú o producto con ese nombre. Use Editar en el listado para modificarlo.");
          return;
        }
      }
              ...
              <div className="compactList">
                {products.map((p) => (
                  <article key={p.id}>
                    ...
                    <button type="button" disabled={productBusy} onClick={() => del("v2_quote_products", p.id)}>
                      <Trash2 />
                    </button>
                    </div>
                  </article>
