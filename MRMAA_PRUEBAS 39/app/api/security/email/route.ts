...
export async function POST(req: NextRequest) {
 try {
  ...
  try {
   await transport.sendMail(brandMail({from:config.from,to:user.email!,subject:en?'UnoMesa · Verification code':'UnoMesa · Código de verificación',
    text:en?`Your UnoMesa code is: ${code}\n\nIt expires in 10 minutes and can only be used once. Never share this code. If you did not request it, review your password.`:
     `Su código de UnoMesa es: ${code}\n\nVence en 10 minutos y solo puede utilizarse una vez. No comparta este código. Si no lo solicitó, revise su contraseña.`}));
  } catch (error) {
   const failure = error as {code?:string;responseCode?:number};
   console.error('unomesa.security.email', {stage:'smtp_delivery',code:failure.code || 'SMTP_ERROR',responseCode:failure.responseCode});
   return reply({error:'EMAIL_DELIVERY_FAILED'},503);
  } finally { transport.close(); }
  return reply({sent:true});
 } catch (error) { return reply({error:error instanceof Error && error.message==='EMAIL_CONFIG'?'EMAIL_CONFIG':'EMAIL_UNAVAILABLE'},503); }
}
...
async function accountEmail(body: Record<string, any>, admin: ReturnType<typeof serverClients>['admin'], actor: string, currentEmail: string, session: string, accessToken: string) {
 ...
 const send = async(to:string,subject:string,text:string) => {
  ...
  try {await transport.sendMail(brandMail({from:config.from,to,subject,text}));} finally {transport.close();}
 };
 const en = body.language==='en';
 if (action === 'account_email_start') {
  ...
  const instructions=en?'Enter this code only in the UnoMesa session where you requested the email change. It expires in 10 minutes. Never share it. If you did not request this, ignore this email; your email address has not changed.':'Introduzca este código únicamente en la sesión de UnoMesa donde solicitó cambiar el correo. Vence en 10 minutos. No lo comparta. Si no lo solicitó, ignore este mensaje; su correo de acceso no ha cambiado.';
  const sent=await Promise.allSettled([
   send(currentEmail,en?'UnoMesa · Authorize email change':'UnoMesa · Autorizar cambio de correo',`${en?'Current email code':'Código del correo actual'}: ${oldCode}\n\n${en?'Requested new email':'Nuevo correo solicitado'}: ${email}\n\n${instructions}`),
   send(email,en?'UnoMesa · Verify your new email':'UnoMesa · Verificar correo nuevo',`${en?'New email code':'Código del correo nuevo'}: ${newCode}\n\n${instructions}`),
  ]);
 ...
 // SQL has already blocked other sessions in UnoMesa atomically. Also revoke
 // their Auth refresh sessions; a provider failure must not undo the change.
 try {await admin.auth.admin.signOut(accessToken,'others');} catch { /* UnoMesa revocations remain effective. */ }
 // Notification failure cannot undo an already completed account change.
 let notified=true;
 try {await send(c.old_email,en?'UnoMesa · Email address changed':'UnoMesa · Correo de acceso actualizado',en?`Your UnoMesa login email was changed to ${c.new_email}. Other sessions have been closed. If you did not make this change, contact support@unomesa.com.`:`Su correo de acceso a UnoMesa cambió a ${c.new_email}. Se cerraron las demás sesiones. Si no realizó este cambio, contacte a support@unomesa.com.`);} catch {notified=false;}
...
async function notifySupport(body: Record<string, any>, admin: ReturnType<typeof serverClients>['admin'], client: ReturnType<typeof serverClients>['client'], actor: string) {
 ...
 const config=smtpConfig("support"), claim=new Date().toISOString();
 ...
 try {
  ...
  const en=body.language==='en';
  await transport.sendMail(brandMail({from:config.from,to:'support@unomesa.com',subject:en?'UnoMesa · Authorized support request':'UnoMesa · Solicitud de soporte autorizada',text:
