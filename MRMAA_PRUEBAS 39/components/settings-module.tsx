...
function AccountSettings({ restaurantId }: { restaurantId: string }) {
  ...
  const confirmationText = language === "en" ? "DELETE UNOMESA" : "ELIMINAR UNOMESA";
  ...
  return <div className="moduleStack">
    <section className="moduleCard settingsPanel">
      ...
      <div className="formStack accountEmailFlow" translate="no">
        ...
        <small>{emailText("Si ya no tiene acceso al correo actual, contacte a support@unomesa.com para revisar su identidad.","If you no longer have access to your current email, contact support@unomesa.com for an identity review.")}</small>
      </div>
    </section>
    <section className="moduleCard settingsPanel">
...
function SupportAuthorization({ restaurantId }: { restaurantId: string }) {
  ...
  return <section className="moduleCard settingsPanel supportPanel" translate="no">
    ...
    <p>{copy('Autorice al equipo de soporte de UnoMesa para revisar este restaurante durante 24 horas. Puede revocar el acceso cuando lo desee. No consume un usuario de su plan.', 'Authorize the UnoMesa support team to review this restaurant for 24 hours. Revoke access at any time. This does not use a plan seat.')}</p>
