...
test('SMTP uses UnoMesa only; old senders rejected; email text preserved', () => {
  ...
  assert.equal(mail.mailSender('passcode'),'UnoMesa <passcode@unomesa.com>');
  assert.equal(mail.mailSender('support'),'UnoMesa <support@unomesa.com>');
  ...
  env.UNOMESA_SUPPORT_FROM='support@mrmaa.com';
  assert.throws(()=>mail.mailSender('support'), /EMAIL_CONFIG/);
