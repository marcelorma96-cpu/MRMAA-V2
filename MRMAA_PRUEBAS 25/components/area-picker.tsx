...
export function AreaPicker({ areas, value, areaId, onChange, required = false, searchOnly = false }: {
  areas: AreaOption[]; value: string; areaId?: string;
  onChange: (name: string, id: string) => void; required?: boolean; searchOnly?: boolean;
}) {
  ...
  useEffect(()=>{
    input.current?.setCustomValidity(search.trim() && !selected
      ? (en?'Select an area from the suggestions.':'Seleccione un área de las sugerencias.') : '');
  },[search,selected,en]);
  ...
  return <span style={{display:'grid',gap:6}}>
    <input ref={input} type="search" required={searchOnly && required} role="combobox" aria-autocomplete="list"
    ...
    {!searchOnly && <select aria-label={en?'Area list':'Listado de áreas'} required={required} value={selected?.id || ''}
      onChange={e=>choose(areas.find(a=>a.id===e.target.value))}>
      <option value="">{en?'Select area':'Seleccionar área'}</option>
      {areas.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}
    </select>}
    <small>{searchOnly
      ? (en?'Type and select a matching area.':'Escriba y seleccione un área de las coincidencias.')
      : (en?'Search and select a suggestion, or choose from the list. Both fields show the same area.':'Busque y seleccione una sugerencia, o escoja del listado. Ambos campos muestran la misma área.')}</small>
