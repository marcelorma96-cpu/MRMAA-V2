...
export function AiAssistant({restaurantId,onTutorial,page}:{restaurantId:string;onTutorial:()=>void;page?:string}){
 ...
 return <div translate="no">
  <button ref={launcher} type="button" className="tutorialLauncher" onClick={()=>setOpen(true)}><MessageCircle/><span>{copy('Asistente MRMAA','MRMAA Assistant')}</span></button>
  {open&&typeof document!=='undefined'&&createPortal(<aside translate="no" className="aiHelpPanel" role="dialog" aria-label={copy('Asistente de ayuda MRMAA','MRMAA help assistant')} onKeyDown={event=>{if(event.key==='Escape')close();}}>
   <header><div><strong>{copy('Asistente MRMAA','MRMAA Assistant')}</strong><small>{copy('Ayuda para usar su aplicación','Help with using your application')}</small></div><button type="button" onClick={close} aria-label={copy('Cerrar','Close')}><X/></button></header>
   ...
   <div className="aiHelpLog" ref={log} role="log" aria-live="polite" aria-relevant="additions">
    {!messages.length&&<div className="aiHelpWelcome"><h3>{copy('¿En qué paso necesita ayuda?','Which step do you need help with?')}</h3><p>{copy('Explico las funciones de MRMAA. No consulto ni modifico registros de su restaurante.','I explain MRMAA features. I do not access or edit your restaurant records.')}</p>
   ...
   <div className="aiHelpCompose">
    ...
    <form onSubmit={event=>{event.preventDefault();void submit();}}>
     <textarea ref={inputRef} value={question} maxLength={1000} rows={3} disabled={busy} aria-label={copy('Su pregunta','Your question')} placeholder={copy('Pregunte cómo usar MRMAA…','Ask how to use MRMAA…')} onChange={event=>{setQuestion(event.target.value);pending.current=null;}}/>
