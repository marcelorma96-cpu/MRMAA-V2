...
export function useUnsavedChanges(dirty: boolean, discard: () => void) {
  ...
  // Successful saves can be followed immediately by navigation, before React
  // has run the effect that removes the now-clean draft. Let callers clear the
  // global registry synchronously once persistence has been confirmed.
  return () => drafts.delete(id.current);
}
