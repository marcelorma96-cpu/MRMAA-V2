...
export function ReservationsEnhanced({
  ...
  function exportRows() {
    ...
    const data = filtered.map((r) => translateRecord({
      Fecha: r.event_date,
  ...
  async function importRows(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const existingResult = await supabase.from("v2_reservations")
        .select("id,client_name,phone,event_date,event_time,area,guests,menu,deposit,payment_method,status,notes,client_id")
        .eq("restaurant_id", restaurantId);
      if (existingResult.error) throw existingResult.error;
      const keyOf = (row: any) => `${String(row.client_name || "").trim().toLowerCase()}|${row.event_date}|${String(row.event_time || "").slice(0, 5)}`;
      const comparable = (row: any) => JSON.stringify({
        phone: row.phone || "", area: row.area || "", guests: Number(row.guests || 0), menu: row.menu || "",
        deposit: Number(row.deposit || 0), payment_method: row.payment_method || "", status: row.status || "", notes: row.notes || "",
      });
      const existing = new Map((existingResult.data || []).map((row: any) => [keyOf(row), row]));
      const creates: any[] = [], updates: { id: string; row: any }[] = [];
      let unchanged = 0;
      payload.forEach((row) => {
        const match: any = existing.get(keyOf(row));
        if (!match) creates.push(row);
        else if (comparable(match) === comparable(row)) unchanged++;
        else {
          const { subtotal, discount_pct, tip_pct, total, balance, restaurant_id, ...editable } = row;
          updates.push({ id: match.id, row: editable });
        }
      });
      for (let start = 0; start < creates.length; start += 250) {
        const result = await supabase.from("v2_reservations").insert(creates.slice(start, start + 250));
        if (result.error) throw result.error;
      }
      for (let start = 0; start < updates.length; start += 25) {
        const results = await Promise.all(updates.slice(start, start + 25).map(({ id, row }) =>
          supabase.from("v2_reservations").update(row).eq("restaurant_id", restaurantId).eq("id", id)));
        const failed = results.find((result) => result.error);
        if (failed?.error) throw failed.error;
      }
      setNotice(`Importación terminada: ${creates.length} nuevas, ${updates.length} actualizadas, ${unchanged} sin cambios y ${omitted.length} omitidas.`);
      await reload();
    } catch (err: any) {
