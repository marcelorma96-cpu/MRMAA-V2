...
function AccountSettings({ onLanguageDirty }: { onLanguageDirty: (dirty: boolean) => void }) {
  ...
  return <div className="moduleStack">
    <EmailLanguagePreference panel onDirtyChange={onLanguageDirty} />
...
export function EnhancedSettings({
    ...
    if (r.data)
      setForm((old: any) => {
        const loaded = {
          ...old,
          ...r.data,
          language: appLanguage(r.data.language),
          currency: appCurrency(r.data.currency),
        ...
        setPreferences(loaded.language, loaded.currency);
        return loaded;
      });
    setAreas((a.data || []) as Area[]);
    setEmployees((e.data || []) as Employee[]);
  ...
  const pendingDraft = tab === "horarios"
    ...
    : tab === "cotizaciones"
      ...
      : false;
  const settingsDirty = sectionChanged || pendingDraft || languageDirty;
  ...
  return (
    <div className="settingsLayout">
        ...
        {tabs.map((x) => (
          <button
            ...
            onClick={() => {
              if (x === tab) return;
              if (settingsDirty) {
                if (!window.confirm("Hay cambios de configuración sin guardar. ¿Desea descartarlos y continuar?")) return;
                discardCurrentSection(tab);
              }
              setTab(x);
            }}
            key={x}
      ...
      {tab === "usuarios" ? (
        ...
        <AccountSettings onLanguageDirty={setLanguageDirty} />
      ) : tab === "seguridad" ? (
