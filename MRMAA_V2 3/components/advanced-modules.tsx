...
export function MonthlySchedules({ restaurantId }: { restaurantId: string }) {
  ...
  async function deletePeriod() {
    if (!form.employee_id || !form.work_date || !form.work_date_to)
      return setNotice("Seleccione empleado y rango de fechas.");
    const employee = employees.find((x) => x.id === form.employee_id);
    if (!window.confirm(`¿Borrar todos los horarios de ${employee?.name || "este empleado"} entre ${form.work_date} y ${form.work_date_to}?`)) return;
    const r = await supabase.from("v2_schedules").delete()
      .eq("restaurant_id", restaurantId).eq("employee_id", form.employee_id)
...
export function EnhancedSettings({
  ...
  const settingsDirty = Boolean(savedSignature && JSON.stringify(form) !== savedSignature);
  useEffect(() => {
