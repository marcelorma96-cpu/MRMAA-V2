import { quotePageEnd, quoteSourceRanges } from "./pdf-pagination";
...
export async function captureQuotePreviewPages(element: HTMLElement): Promise<QuotePageImage[]> {
  ...
  const canvas = await html2canvas(element, {
    ...
    onclone: (_document, paper) => {
      ...
      // Size the numeric columns from their actual font metrics, including
      // large amounts and currencies. A fixed 100px column could clip totals.
      const rows = Array.from(paper.querySelectorAll<HTMLElement>(".previewTable > div"));
      const numericWidths = [2, 3, 4].map((column, index) => Math.max(index === 0 ? 60 : 100,
        ...rows.map(row => {
          const cell = row.children[column];
          if (!cell) return 0;
          const range = _document.createRange();
          range.selectNodeContents(cell);
          return Math.ceil(range.getBoundingClientRect().width) + 4;
        })));
      rows.forEach(row => {
        row.style.gridTemplateColumns = `minmax(0,1fr) minmax(0,1.4fr) ${numericWidths.map(width => `${width}px`).join(" ")}`;
      });
...
export async function downloadQuotePreviewPdf(
  ...
  const title = `${currentAppLanguage() === "en" ? "Quote" : "Cotizacion"}-${quoteNumber}`;
