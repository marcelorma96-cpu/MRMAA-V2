...
import { startLandingPixel, startRegistrationPixel, trackMetaRegistration } from "@/lib/meta-pixel";
import { startRegistrationAnalytics, trackRegistration as trackVercelRegistration } from "@/lib/registration-analytics";
import { startLandingGoogleAds, startRegistrationGoogleAds, trackGoogleAdsRegistration } from "@/lib/google-ads";
...
function trackRegistration(name: Parameters<typeof trackVercelRegistration>[0], reason?: string) {
  trackVercelRegistration(name, reason);
  trackMetaRegistration(name);
  if (name === "Registro_exitoso") trackGoogleAdsRegistration();
}
...
function HomeContent() {
  ...
  const [session, setSession] = useState<Session | null>(null),
    ...
    [selectedPlan, setSelectedPlan] = useState<PlanCode>("advanced"),
  ...
  return (
    <main className="expired">
      <section style={{ maxWidth: 920 }}>
        ...
        {account.deletionScheduledAt && <p role="alert" translate="no">{en ? "If you do not reactivate your subscription, this restaurant’s data will be deleted on " : "Si no reactiva la suscripción, los datos de este restaurante se eliminarán el "}{new Intl.DateTimeFormat(en ? "en-US" : "es-GT", { dateStyle: "long" }).format(new Date(account.deletionScheduledAt))}.</p>}
  ...
  return (
    <main className="auth">
      <section className="authBrand">
        ...
        <p>
          {reason === "reset"
            ? "Cree una nueva contraseña para UnoMesa."
            : "Cree la contraseña con la que ingresará a UnoMesa."}
        </p>
      </section>
...
function Landing({ startTrial, login }: { startTrial: (p: PlanCode, interval?: BillingInterval) => void; login: () => void }) {
  ...
  useEffect(() => startLandingGoogleAds(), []);
  ...
  const features = [
    [ShieldCheck, copy("Ayuda cuando la necesita", "Help when you need it"), copy("Tutorial paso a paso sin límite y asistente de IA en español e inglés para explicar cómo usar UnoMesa. 50 consultas al mes compartidas por restaurante.", "Unlimited step-by-step tutorial and an AI assistant in Spanish and English to explain how to use UnoMesa. 50 questions per month shared by each restaurant."), copy("Todos los planes", "All plans")],
  ...
  const faqs = [
    [copy("¿Qué incluye la prueba?", "What does the trial include?"), copy("10 días de Advanced sin tarjeta. Al terminar, puede contratar Basic, Intermediate o Advanced para continuar.", "10 days of Advanced without a card. When your trial ends, choose Basic, Intermediate or Advanced to continue.")],
    ...
    [copy("¿Cómo aprendo a usarlo?", "How do I learn to use it?"), copy("El tutorial dentro de UnoMesa tiene un índice por temas, instrucciones paso a paso y consejos para evitar errores. Puede volver a abrirlo cuando lo necesite.", "The tutorial inside UnoMesa includes a topic index, step-by-step instructions and tips to avoid mistakes. Reopen it whenever you need.")],
  ...
  return <main className="landing" translate="no">
    <LandingAnalytics />
    <nav className="landingNav"><a className="landingLogo" href="#top"><BrandLogo /></a>
      ...
      <div className="landingAuthActions">
        ...
        {PUBLIC_SIGNUP_ENABLED && <button type="button" className="primary" onClick={() => startTrial("advanced", interval)}>{copy("Pruebe gratis", "Try it free")}</button>}
      </div></div></nav>
    <section className="hero" id="top"><div><span className="heroTag">{copy("Administración para restaurantes", "Restaurant management")}</span>
      <h1>{copy("Menos desorden.", "Less clutter.")}<br/><em>{copy("Más control de su restaurante.", "More control of your restaurant.")}</em></h1>
      <p>{copy("Reservas, cotizaciones, clientes y horarios en un solo lugar. Consulte quién llega, qué ha pagado y quién trabaja, sin buscar entre archivos y mensajes.", "Reservations, quotes, customers and schedules in one place. See who is coming, what they have paid and who is working, without searching through files and messages.")}</p>
      <div className="heroActions"><button className="primary" onClick={() => PUBLIC_SIGNUP_ENABLED ? startTrial("advanced") : login()}>{copy(PUBLIC_SIGNUP_ENABLED ? "Pruebe gratis 10 días" : "Iniciar sesión", PUBLIC_SIGNUP_ENABLED ? "Try free for 10 days" : "Sign in")}<ArrowRight/></button><a className="landingSecondary" href="#funciones">{copy("Explorar funciones", "Explore features")}</a></div><p className="trialReassurance">{copy("10 días de Advanced gratis · Sin tarjeta · Elija su plan después", "10 free days of Advanced · No card required · Choose your plan later")}</p></div>
      <div className="heroPanel landingPreview"><div className="miniTop"><BrandLogo className="brandLogoMini" /><span>{copy("Vista de ejemplo", "Example preview")}</span></div>
        ...
        <div className="previewContent" aria-live="polite">
          ...
          <p>{preview === 2 ? copy("Salón · Horarios del personal", "Dining room · Staff schedules") : copy("Terraza · 24 invitados", "Terrace · 24 guests")}</p>
    ...
    <section className="landingTeamSchedules" aria-labelledby="team-schedules-heading">
      <div className="landingTeamCopy">
        <span className="heroTag">{copy("Horarios del personal", "Staff schedules")}</span>
        <h2 id="team-schedules-heading">{copy("Organice los horarios de su equipo.", "Organize your team's schedules.")}</h2>
        <p>{copy("Asigne turnos, descansos y permisos. Consulte el calendario mensual por área y comparta los horarios en PDF o Excel.", "Assign shifts, days off and leave. View the monthly calendar by area and share schedules as PDF or Excel.")}</p>
        <ul>{[copy("Turnos y comidas en un solo calendario", "Shifts and meal breaks in one calendar"), copy("Copie horarios para preparar las próximas fechas", "Copy schedules to prepare upcoming dates"), copy("Colores para identificar cada asignación", "Colors to identify each assignment")].map(text => <li key={text}><Check aria-hidden="true"/>{text}</li>)}</ul>
        <p className="landingTeamAvailability">{copy("Disponible en Intermediate y Advanced. Incluido en la prueba gratuita de Advanced.", "Available in Intermediate and Advanced. Included in the free Advanced trial.")}</p>
        <button type="button" className="primary" onClick={() => PUBLIC_SIGNUP_ENABLED ? startTrial("advanced") : login()}>{copy(PUBLIC_SIGNUP_ENABLED ? "Probar horarios gratis" : "Iniciar sesión", PUBLIC_SIGNUP_ENABLED ? "Try schedules free" : "Sign in")}<ArrowRight aria-hidden="true"/></button>
      </div>
      <figure className="landingTeamExample">
        <figcaption><strong>{copy("Su equipo, de un vistazo", "Your team at a glance")}</strong><span>{copy("Ejemplo ilustrativo · Datos ficticios", "Illustrative example · Sample data")}</span></figcaption>
        <div className="landingTeamTableWrap">
          <table><caption>{copy("Salón · Ejemplo de tres días", "Dining room · Three-day example")}</caption>
            <thead><tr><th scope="col">{copy("Empleado", "Employee")}</th>{[copy("Lun", "Mon"), copy("Mar", "Tue"), copy("Mié", "Wed")].map(day => <th scope="col" key={day}>{day}</th>)}</tr></thead>
            <tbody>
              <tr><th scope="row">Ana</th><td><span className="teamShift">11:00–19:00</span></td><td><span className="teamShift">11:00–19:00</span></td><td><span className="teamRest">{copy("Descanso", "Day off")}</span></td></tr>
              <tr><th scope="row">Luis</th><td><span className="teamShift">14:00–22:00</span></td><td><span className="teamRest">{copy("Descanso", "Day off")}</span></td><td><span className="teamShift">14:00–22:00</span></td></tr>
              <tr><th scope="row">Sofía</th><td><span className="teamLeave">{copy("Permiso", "Leave")}</span></td><td><span className="teamShift">11:00–19:00</span></td><td><span className="teamShift">11:00–19:00</span></td></tr>
            </tbody>
          </table>
        </div>
        <p>{copy("Vista mensual · Organización por área · PDF y Excel", "Monthly view · Organized by area · PDF and Excel")}</p>
      </figure>
    </section>
    <section className="featuresSection" id="funciones"><div className="sectionIntro"><span>{copy("Herramientas que trabajan juntas", "Tools that work together")}</span><h2>{copy("Menos tareas sueltas. Más claridad.", "Fewer scattered tasks. More clarity.")}</h2></div>
      <div className="featureGrid">{[...features.slice(1, 3), features[4], features[5], features[3], features[0], ...features.slice(6)].map(([Icon,title,body,plan]) => <article key={title}><Icon/><small className="landingPlanLabel">{plan}</small><h3>{title}</h3><p>{body}</p></article>)}</div></section>
    <section className="plansSection" id="planes"><div className="sectionIntro"><span>{copy("Planes simples", "Simple plans")}</span><h2>{copy("Elija lo que necesita hoy.", "Choose what you need today.")}</h2><p>{copy("Empiece con 10 días de Advanced gratis. Después elija su plan. Precios en USD; el pago anual cubre 12 meses por el precio de 10 mensualidades.", "Start with 10 free days of Advanced, then choose your plan. Prices in USD; annual billing covers 12 months for the price of 10 monthly payments.")}</p></div>
      <PlanCards interval={interval} onInterval={setInterval} onChoose={() => PUBLIC_SIGNUP_ENABLED && startTrial("advanced")} trialAdvanced signup busy={!PUBLIC_SIGNUP_ENABLED}/></section>
    ...
    <section className="landingFinal"><span className="heroTag">{copy("Administración para restaurantes", "Restaurant management")}</span><h2>{copy("Pruebe UnoMesa con su próxima reserva.", "Try UnoMesa with your next reservation.")}</h2><p>{copy("Cree su cuenta, registre una reserva y explore las herramientas de Advanced durante 10 días. Sin tarjeta y con ayuda dentro de la aplicación.", "Create your account, add a reservation and explore Advanced tools for 10 days. No card required, with help inside the app.")}</p><button className="primary" onClick={() => PUBLIC_SIGNUP_ENABLED ? startTrial("advanced") : login()}>{copy(PUBLIC_SIGNUP_ENABLED ? "Comenzar mi prueba" : "Iniciar sesión", PUBLIC_SIGNUP_ENABLED ? "Start my free trial" : "Sign in")}<ArrowRight/></button></section>
    <footer className="landingFooter"><div className="landingLogo"><BrandLogo /></div><p>{copy("Administración para restaurantes", "Restaurant management")}</p><a href="mailto:support@unomesa.com">support@unomesa.com</a><div className="legalLinks"><a href="/terminos">{copy("Términos", "Terms")}</a><a href="/privacidad">{copy("Privacidad", "Privacy")}</a><a href="/suscripciones">{copy("Suscripciones y reembolsos", "Subscriptions and refunds")}</a><a href="/tratamiento-datos">{copy("Tratamiento de datos", "Data processing")}</a></div></footer>
  </main>;
}
...
function AccessForm({
  ...
  const { language, currency, setPreferences, t } = useAppPreferences();
  ...
  const [form, setForm] = useState({
    ...
    [signupPlan] = useState<PlanCode>("advanced"),
    [signupInterval] = useState<BillingInterval>("month"),
  ...
  useEffect(() => {
    setForm(current => current.language === language && current.currency === currency
      ? current : { ...current, language, currency });
  }, [language, currency]);
  useEffect(() => {
  ...
  const registrationOpen = useRef(false);
  const invalidReported = useRef(false);
  useEffect(() => {
    if (currentMode !== "signup" || showRecovery) { registrationOpen.current = false; return; }
    const stop = startRegistrationAnalytics();
    const stopMeta = startRegistrationPixel();
    const stopGoogle = startRegistrationGoogleAds();
    if (!registrationOpen.current) {
      registrationOpen.current = true;
      trackRegistration("Registro_abierto");
    }
    return () => { stop(); stopMeta(); stopGoogle(); };
  }, [currentMode, showRecovery]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (currentMode === "signup") trackRegistration("Registro_intento");
    const reject = (message: string, reason: string) => {
      if (currentMode === "signup") trackRegistration("Registro_error", reason);
      return setFeedback(message);
    };
    if (!configured)
      return reject("Faltan las variables de Supabase en Vercel.", "configuration");
    if (currentMode === "signup" && !PUBLIC_SIGNUP_ENABLED)
      return reject("El registro público está temporalmente deshabilitado. Ingrese con una cuenta existente o mediante una invitación.", "closed");
    if (currentMode === "signup" && form.password !== form.confirmPassword)
      return reject("Las contraseñas no coinciden.", "password_mismatch");
    if (currentMode === "signup" && !passwordIsStrong(form.password))
      return reject("Use 8 caracteres o más, con mayúscula, minúscula, número y carácter especial.", "password_policy");
    if (currentMode === "signup" && !accepted) return reject("Debe aceptar los Términos y la Política de privacidad.", "terms");
    if (TURNSTILE_SITE_KEY && !captchaToken)
      return reject("Complete la verificación de seguridad.", "captcha");
    ...
    if (currentMode === "login") {
      ...
      if (!response.ok || result.ok !== true) {
        trackRegistration("Registro_error", result.code === "REGISTRATION_IN_PROGRESS" ? "pending" : response.status === 429 ? "rate_limit" : "server");
        ...
        trackRegistration("Registro_exitoso");
      ...
      if (currentMode === "signup") trackRegistration("Registro_error", "network");
      setFeedbackKind("error");
  ...
  return (
    <main className={`accessPage ${currentMode === "signup" && !showRecovery ? "signupPage" : ""}`}>
      ...
      <section className="accessContent">
        <form
          ...
          onInvalidCapture={(event) => {
            ...
            if (currentMode !== "signup" || showRecovery || invalidReported.current) return;
            invalidReported.current = true;
            trackRegistration("Registro_intento");
            trackRegistration("Registro_error", "browser_validation");
            // Native validation reports every invalid field in the same turn.
        ...
        >
          <h2 ref={accessHeading} tabIndex={-1} translate="no">
            {t(showRecovery
          ...
          {!showRecovery && currentMode === "signup" && <p className="signupIntro">Cree su cuenta para comenzar. No se realiza ningún cobro al registrarse.</p>}
          {feedbackKind === "success" && feedbackNotice}
          {showRecovery && (
            <p>
              Escriba el correo electrónico asociado con su cuenta de UnoMesa.
            </p>
          )}
          {!showRecovery && currentMode === "signup" && (
            <>
              ...
              <details className="signupOptions"><summary>Personalizar país, idioma y moneda</summary><div className="grid2">
                <label>
                  País
                  <input
                    required
              ...
              <div className="chosenPlan"><span>Su prueba gratuita</span><strong>Advanced · 10 días · Sin tarjeta</strong><small>Al terminar, elija Basic, Intermediate o Advanced para continuar. No se cobra al registrarse.</small></div>
            </>
          )}
          <label>
            Correo electrónico
          ...
          {!showRecovery && currentMode === "signup" && <label>
            Confirmar contraseña
            <PasswordInput
              required
              minLength={8}
              ...
              aria-describedby={form.confirmPassword && form.password !== form.confirmPassword ? "signup-password-mismatch" : undefined}
            />
            {form.confirmPassword && form.password !== form.confirmPassword &&
              <small id="signup-password-mismatch" className="passwordMismatch" role="status">Las contraseñas no coinciden.</small>}
          </label>}
          ...
          <p className="authLegal" translate="no">{language === "en" ? "By signing in, you acknowledge the current " : "Al ingresar, reconoce los "}<a href="/terminos">{t("Términos")}</a>{language === "en" ? " and " : " y la "}<a href="/privacidad">{t("Política de Privacidad")}</a>{language === "en" ? "." : " vigentes."}</p>
        </form>
      </section>
    </main>
  );
