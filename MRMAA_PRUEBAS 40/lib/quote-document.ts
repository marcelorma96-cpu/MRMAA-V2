...
export function createQuoteDocument(pages: readonly QuotePageImage[], title: string, print = false) {
  ...
  pages.forEach((page, index) => {
    if (!/^data:image\/(?:png|jpeg);base64,[A-Za-z0-9+/=]+$/.test(page.image)
    ...
    pdf.addImage(page.image, page.image.startsWith("data:image/jpeg;") ? "JPEG" : "PNG", page.x, page.y, page.width, page.height);
  });
