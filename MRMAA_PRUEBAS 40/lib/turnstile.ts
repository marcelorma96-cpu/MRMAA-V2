...
export function loadTurnstile(): Promise<TurnstileApi> {
  ...
  const request = new Promise<TurnstileApi>((resolve, reject) => {
    ...
    const ready = () => {
      ...
      if (!api || typeof api.render !== "function") { fail(); return; }
      // The script's load event already guarantees that its API is available.
      // Turnstile rejects ready() when api.js was loaded with async/defer.
      complete = true;
      cleanup();
      resolve(api);
    };
