...
export function buildBackupWorkbook(data: Record<string, unknown[]>, english = false) {
  ...
  const overview: (string | number)[][] = [
    ["UnoMesa", english ? "Data export" : "Exportación de datos"],
...
export function downloadBackupExcel(data: Record<string, unknown[]>, english = false) {
  XLSX.writeFile(buildBackupWorkbook(data, english), `UnoMesa-${english ? "backup" : "respaldo"}-${new Date().toISOString().slice(0, 10)}.xlsx`, { compression: true });
}
