...
export function SecurityCenter({ restaurantId, exportOnly = false }: { restaurantId: string; exportOnly?: boolean }) {
  ...
  async function exportBackup(format: "xlsx" | "json") {
    ...
    try {
      ...
      link.href = url;
      link.download = `UnoMesa-respaldo-${date}.json`;
      link.click();
...
export function SessionManager({restaurantId}:{restaurantId?:string}) {
 ...
 const load=useCallback(async(silent=false)=>{const g=++generation.current;setBusy(true);if(!silent){setNotice('');setRows([]);}
  const r=await supabase.rpc('v2_session_list',{p_restaurant:team?restaurantId:null,p_after:after,p_search:term});
  if(g!==generation.current)return;
  setBusy(false);
  if(r.error){setRows([]);setNotice(en?'Could not load sessions. Retry.':'No se pudieron cargar las sesiones. Reintente.');return;}
  const data=(r.data||[]) as ActiveSession[];setNotice('');setRows(data.slice(0,50));setMore(data.length>50);
 },[team,restaurantId,after,term,en]);
 useEffect(()=>{
  void load();
  const refresh=()=>{if(document.visibilityState==='visible'&&!lock.current)void load(true);};
  const timer=setInterval(refresh,60000);
  window.addEventListener('focus',refresh);document.addEventListener('visibilitychange',refresh);
  return()=>{clearInterval(timer);window.removeEventListener('focus',refresh);document.removeEventListener('visibilitychange',refresh);generation.current++;};
 },[load]);
 async function revoke(row?:ActiveSession){
  if(lock.current)return;
  if(!confirmApp(row?(en?'End this session? This browser must sign in again to UnoMesa, including any other restaurant accessed with the same session.':'¿Cerrar esta sesión? Ese navegador deberá ingresar nuevamente a UnoMesa, incluso a otros restaurantes abiertos con la misma sesión.'):(en?'End all your other sessions? This session will stay open.':'¿Cerrar todas sus demás sesiones? Esta permanecerá abierta.')))return;
  ...
  try{
   ...
   if(row?.current){await signOutCurrentSession();return;}
 ...
 return <section className="moduleCard sessionManager" translate="no">
  ...
  <p>{en?'View browsers signed in to UnoMesa. Ending a session does not permanently block the device.':'Consulte los navegadores con acceso a UnoMesa. Cerrar una sesión no bloquea el dispositivo permanentemente.'}</p>
  <p className="muted">{en?'Only sessions with access are shown. Sessions expire after one hour without activity in UnoMesa. The list refreshes every minute; an active session does not mean someone is online right now. Device names are approximate.':'Solo se muestran sesiones con acceso. Caducan tras una hora sin actividad en UnoMesa. La lista se actualiza cada minuto; una sesión activa no significa que alguien esté conectado en ese momento. Los nombres de dispositivos son aproximados.'}</p>
  <div className="rowActions">
