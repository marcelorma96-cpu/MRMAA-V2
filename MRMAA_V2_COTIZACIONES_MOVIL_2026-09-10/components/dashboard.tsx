...
export function Dashboard({
  ...
  const canOperate = isAdmin || ["gerente", "operacion"].includes(memberRole);
  const canManageSchedules = isAdmin || memberRole === "gerente";
          ...
          {visible.map((q) => (
            <div className={`tr quoteRow ${selected.includes(q.id) ? "selectedRecord" : ""}`} key={q.id}>
              ...
              <div className="rowActions">
                ...
                <button className="quoteDeleteAction" title="Enviar a la papelera" aria-label={`Enviar cotización ${q.quote_number} a la papelera`} onClick={() => remove(q)}>
                  <Trash2 />
                </button>
              </div>
            </div>
