...
create index if not exists v2_clients_trash_idx on public.v2_clients(restaurant_id,deleted_at desc) where deleted_at is not null;
create index if not exists v2_quotes_trash_idx on public.v2_quotes(restaurant_id,deleted_at desc) where deleted_at is not null;
create index if not exists v2_reservations_trash_idx on public.v2_reservations(restaurant_id,deleted_at desc) where deleted_at is not null;
...
begin
  ...
  if deleted_time is null or deleted_time > now()-interval '365 days' then raise exception 'El registro debe permanecer 365 días en la papelera'; end if;
  perform set_config('mrmaa.recycle_operation','allowed',true);
