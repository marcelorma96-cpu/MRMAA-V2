...
export function ReservationsEnhanced({
  ...
  function print() {
    ...
    const table =
      printMode !== "summary"
        ? `<table><thead><tr><th>Fecha</th><th>Hora</th><th>Cliente</th><th>Área</th><th>Inv.</th><th>Menú</th><th>Anticipo</th><th>Método</th><th>Estado</th><th>Observaciones</th></tr></thead><tbody>${filtered.map((r) => `<tr><td>${escape(r.event_date)}</td><td>${escape(r.event_time?.slice(0, 5))}</td><td>${escape(r.client_name)}<small>${escape(r.phone)}</small></td><td>${escape(r.area)}</td><td>${r.guests}</td><td>${escape(r.menu)}</td><td>${escape(money(Number(r.deposit || 0)))}</td><td>${escape(paymentMethodLabel(r.payment_method))}</td><td>${escape(r.status)}</td><td>${escape(r.notes)}</td></tr>`).join("")}</tbody></table>`
        : "";
    printHtml(
            ...
            <article key={r.id}>
              ...
              <div className="reservationMain">
                ...
                <div className="reservationDeposit">
                  <span>
                    <b>Anticipo:</b> {money(Number(r.deposit || 0))}
                  </span>
                  <span>
