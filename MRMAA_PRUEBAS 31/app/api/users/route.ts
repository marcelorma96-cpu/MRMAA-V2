...
function enforceRequestSafety(req: NextRequest, _limit = 30) {
  const origin = req.headers.get("origin");
  const ownOrigin = new URL(req.url).origin;
  if (origin && origin !== ownOrigin && origin !== siteOrigin(ownOrigin))
    throw new RequestSafetyError("Origen de solicitud no permitido.", 403);
}
...
async function context(req: NextRequest) {
  if (!url || !anon || !service)
    throw new Error(
      "El servicio no está disponible. Intente nuevamente más tarde.",
    );
  ...
  await requireVerifiedMfa(user, token);
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: userMessage(e) }, { status: e instanceof RequestSafetyError ? e.status : 403 });
  }
}
export async function POST(req: NextRequest) {
  try {
    ...
    const { admin, member } = await context(req),
      { email, name, role, language } = await readBoundedJson(req, 32_000);
    ...
    const siteUrl = siteOrigin(new URL(req.url).origin);
    ...
    const access = await admin.rpc("v2_billing_can_write", { p_restaurant: member.restaurant_id });
    if (access.error || !access.data) throw new Error("Su prueba o suscripción terminó. Active un plan para realizar cambios.");
    const [capacity, count, existingMembership] = await Promise.all([
      admin.rpc("v2_plan_user_limit", { p_restaurant: member.restaurant_id }),
      admin.from("v2_members").select("user_id", {count:"exact",head:true}).eq("restaurant_id",member.restaurant_id).in("status",["activo","invitado"]),
      admin.from("v2_members").select("user_id,status").eq("restaurant_id",member.restaurant_id).eq("email",normalizedEmail).maybeSingle(),
    ]);
    if (capacity.error || count.error || existingMembership.error) throw new Error("No se pudo comprobar el límite de usuarios. Intente nuevamente.");
    if (!existingMembership.data && (count.count || 0) >= capacity.data) throw new Error("Su plan alcanzó el límite de usuarios. Cancele una invitación o cambie de plan.");
    ...
    if (memberResult.error) {
      if (!existingUser) {
        const linked = await admin.from("v2_members").select("user_id", {count:"exact",head:true}).eq("user_id",invitedUser.id);
        if (!linked.error && linked.count === 0) await admin.auth.admin.deleteUser(invitedUser.id);
      }
      throw memberResult.error;
    }
    return NextResponse.json({ ok: true, email: invitedUser.email, existingUser });
  } catch (e: any) {
    return NextResponse.json({ error: userMessage(e) }, { status: 400 });
  }
}
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { admin, member, user } = await context(req),
      { user_id, action, name, role, language } = await readBoundedJson(req, 32_000);
...
export async function DELETE(req: NextRequest) {
  try {
    enforceRequestSafety(req, 10);
    const { admin, member, user } = await context(req),
      { user_id } = await readBoundedJson(req, 32_000);
    await takeDistributedRateLimit(admin, req, "users:delete", 10);
    if (user_id === user.id)
