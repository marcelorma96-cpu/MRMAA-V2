  ...
  const grouped = useMemo(
    () =>
      areas
        .map((a) => ({
          area: a,
          people: employees.filter((e) => e.area_id === a.id),
        }))
        ...
        {grouped.map((g) => (
          <div className="monthlyArea" key={g.area.id}>
            ...
            <div className="monthlyTable">
              ...
              {g.people.map((e) => (
                <div className="monthRow" key={e.id}>
                  <b>{e.name}</b>
                  {Array.from({ length: daysIn(month) }, (_, i) => {
      ...
      <section className="moduleCard settingsPanel">
        ...
        {tab === "clientes" && (
          <p className="mutedText">
            Los clientes se crean manualmente, desde reservaciones o mediante
            importación.
          </p>
        )}
        {tab === "cotizaciones" && (
        ...
        {tab === "reportes" && (
          <p className="mutedText">
            Los reportes utilizan el rango de fechas seleccionado y están
