...
export function ReservationsEnhanced({
  ...
  function print() {
    ...
    const table =
      printMode !== "summary"
        ? `<table><thead><tr><th>Fecha/hora</th><th>Cliente</th><th>Área</th><th>Inv.</th><th>Estado</th><th>Observaciones</th></tr></thead><tbody>${filtered.map((r) => `<tr><td>${escape(r.event_date)} ${escape(r.event_time?.slice(0, 5))}</td><td>${escape(r.client_name)}<small>${escape(r.phone)}</small></td><td>${escape(r.area)}</td><td>${r.guests}</td><td>${escape(r.status)}</td><td>${escape(r.notes)}</td></tr>`).join("")}</tbody></table>`
        : "";
