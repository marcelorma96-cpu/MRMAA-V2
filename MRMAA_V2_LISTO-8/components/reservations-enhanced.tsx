...
export function ReservationsEnhanced({
  ...
  function print() {
    ...
    const table =
      printMode !== "summary"
        ? `<table><thead><tr><th>Fecha</th><th>Hora</th><th>Cliente</th><th>Área</th><th>Inv.</th><th>Estado</th><th>Observaciones</th></tr></thead><tbody>${filtered.map((r) => `<tr><td>${escape(r.event_date)}</td><td>${escape(r.event_time?.slice(0, 5))}</td><td>${escape(r.client_name)}<small>${escape(r.phone)}</small></td><td>${escape(r.area)}</td><td>${r.guests}</td><td>${escape(r.status)}</td><td>${escape(r.notes)}</td></tr>`).join("")}</tbody></table>`
        : "";
    const win = window.open("", "_blank");
    if (!win) return setNotice("Permita ventanas emergentes para imprimir.");
    win.document.write(
