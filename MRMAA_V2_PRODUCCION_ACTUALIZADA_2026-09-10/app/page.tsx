      ...
      <section className="accessContent">
        ...
        >
          ...
          {!showRecovery && <label>
            ...
            />
            <small>Mínimo 8 caracteres; incluya mayúscula, minúscula, número y símbolo.</small>
          </label>}
...
function Turnstile({ onToken }: { onToken: (token: string) => void }) {
  useEffect(() => {
    const callbackName = "mrmaaTurnstileCallback";
    (window as any)[callbackName] = (token: string) => onToken(token);
    const current = document.querySelector<HTMLScriptElement>('script[data-mrmaa-turnstile]');
    if (!current) {
  ...
  return <div className="turnstileWrap"><div className="cf-turnstile" data-sitekey={TURNSTILE_SITE_KEY} data-callback="mrmaaTurnstileCallback" data-theme="auto" /></div>;
}
