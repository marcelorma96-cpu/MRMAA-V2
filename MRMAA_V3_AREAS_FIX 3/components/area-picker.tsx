    ...
    <details>
      ...
      <input type="search" aria-label={en?'Filter available areas':'Filtrar áreas disponibles'} placeholder={en?'Type to filter the list':'Escriba para filtrar el listado'}
        value={search} onChange={e=>{
          ...
          setSearch(text);
          const query=normalizeArea(text);
          const match=query ? areas.find(a=>normalizeArea(a.name)===query)
            || areas.find(a=>normalizeArea(a.name).startsWith(query))
            || areas.find(a=>normalizeArea(a.name).includes(query)) : undefined;
          onChange(match?.name || '',match?.id || '');
        }} />
      <small>{en ? 'Typing selects a matching area from the list. Check the selected area before saving.' : 'Al escribir se selecciona un área coincidente del listado. Revise el área seleccionada antes de guardar.'}</small>
