-- MRMAA V2 — corrección de permisos por rol, 10 de septiembre de 2026.
-- Instalación existente: ejecutar ESTE archivo completo en SQL Editor > Run.
-- No vuelve a instalar módulos ni cambia datos, contraseñas, roles o estados existentes.
-- Seguro de repetir: reemplaza políticas en una transacción (todo o nada).
-- Administrador: acceso completo dentro de su restaurante.
-- Gerente: operación + asignar/copiar/generar/borrar horarios.
-- Operación: crear/editar clientes, reservas y cotizaciones, importar y convertir.
-- Lectura: consultar/exportar/imprimir. Ninguna escritura de datos del restaurante.
-- Configuración, catálogos, usuarios, auditoría y papelera: solo administrador.
...
returns boolean language sql stable security definer set search_path=public as $$
  select exists (
    ...
    where restaurant_id=target_restaurant
      ...
      and status='activo'
      and lower(btrim(role))=any(allowed_roles)
  );
$$;
...
-- Los permisos de tabla y las políticas RLS se aplican conjuntamente.
-- TRUNCATE no está sujeto a RLS: no debe estar concedido al cliente.
do $$
...
begin
  foreach table_name in array array[
    ...
    if table_name in ('v2_members','v2_audit_log') then
      execute format('grant select on table public.%I to authenticated',table_name);
    elsif table_name='v2_restaurants' then
      execute format('grant select, update on table public.%I to authenticated',table_name);
    else
...
create policy v2_areas_write on public.v2_areas for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
create policy v2_employees_write on public.v2_employees for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
create policy v2_shifts_write on public.v2_shifts for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
create policy v2_templates_write on public.v2_message_templates for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
create policy v2_products_write on public.v2_quote_products for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
create policy v2_comms_write on public.v2_communication_settings for all to authenticated using(public.v2_current_admin(restaurant_id)) with check(public.v2_current_admin(restaurant_id));
...
-- Resultado de comprobación: todas las tablas deben mostrar rls_activado=true.
