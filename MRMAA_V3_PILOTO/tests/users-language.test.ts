...
let accountHandlers: typeof import("../app/api/account/route");
before(async () => { handlers = await import("../app/api/users/route"); accountHandlers = await import("../app/api/account/route"); });
let requestNumber = 0;

function environment(options: { existing?: boolean; foreign?: boolean; owner?: boolean; role?: string; status?: string; preferenceFailure?: boolean; authLanguage?: unknown } = {}) {
  ...
  const authUser = {
    ...
    user_metadata: { language: options.authLanguage ?? "en", full_name: "Equipo", preserved: "keep-me" } as Record<string, any>,
  };
...
test("reenviar normaliza preferencias francesas antiguas a español", async () => {
  const env = environment({ authLanguage: "fr" });
  try {
    const result = await handlers.PATCH(request("PATCH", { user_id: "22222222-2222-4222-8222-222222222222" }));
    assert.equal(result.status, 200, JSON.stringify(await result.json()));
    assert.equal(env.authUser.user_metadata.language, "es");
...
for (const role of ["lectura", "operacion", "gerente", "desconocido"]) {
  test(`${role}: rechaza gestión de usuarios y cuenta desde llamadas directas`, async () => {
    const env = environment({ role });
    try {
      const calls = [
        () => handlers.GET(request("GET")),
        () => handlers.POST(request("POST", invitation)),
        () => handlers.PATCH(request("PATCH", { action: "update_member", user_id: "22222222-2222-4222-8222-222222222222", role: "administrador", name: "Equipo" })),
        () => handlers.DELETE(request("DELETE", { user_id: "22222222-2222-4222-8222-222222222222" })),
        () => accountHandlers.POST(request("POST", { action: "transfer", target_user_id: "22222222-2222-4222-8222-222222222222" })),
        () => accountHandlers.POST(request("POST", { action: "delete_account", confirmation: "ELIMINAR MRMAA" })),
      ];
      for (const call of calls) assert.ok((await call()).status >= 400);
      assert.ok(env.calls.every((call) => call.method === "GET" || call.method === "HEAD"));
      assert.ok(!env.calls.some((call) => call.path.startsWith("/auth/v1/admin/") || call.path === "/auth/v1/invite"));
    } finally { env.restore(); }
  });
}
for (const role of ["lectura", "operacion", "gerente", "administrador"]) {
  test(`invitar conserva el rol ${role} en la membresía`, async () => {
    const env = environment();
    try {
      const result = await handlers.POST(request("POST", { ...invitation, role }));
      assert.equal(result.status, 200);
      const membership = env.calls.find((call) => call.path === "/rest/v1/v2_members" && call.method === "POST");
      assert.ok(membership);
      assert.equal(membership.body.role, role);
    } finally { env.restore(); }
  });
}
