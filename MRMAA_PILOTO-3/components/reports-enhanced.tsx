...
export function EnhancedReports({ restaurantId }: { restaurantId: string }) {
  ...
  async function print() {
    try {
      ...
      printHtml(`<html><head><title>${esc(titles[kind])}</title><style>@page{size:A4 landscape;margin:8mm}body{font-family:Arial;color:#18181b}h1{font-family:Georgia}table{width:100%;table-layout:fixed;border-collapse:collapse}th,td{padding:5px 4px;border-bottom:1px solid #ddd;text-align:left;font-size:9px;vertical-align:top;overflow-wrap:anywhere}th{background:#18181b;color:#fff}</style></head><body><h1>${esc(titles[kind])}</h1><p>${esc(from)} a ${esc(to)}</p><table><thead><tr>${columns.map((column) => `<th>${esc(column)}</th>`).join("")}</tr></thead><tbody>${output.map((row) => `<tr>${columns.map((column) => `<td>${esc(row[column])}</td>`).join("")}</tr>`).join("")}</tbody></table></body></html>`);
