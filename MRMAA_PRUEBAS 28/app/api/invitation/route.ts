...
async function invitationContext(req: NextRequest, invitation: unknown) {
  ...
  await takeDistributedRateLimit(admin, req, "invitation:verify", 15);
...
async function body(req: NextRequest) {
  ...
  return readBoundedJson(req, 4000);
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "El enlace ya venció o fue utilizado." }, { status: error instanceof RequestSafetyError ? error.status : 403 });
  }
}
