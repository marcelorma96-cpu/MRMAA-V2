...
export function SecurityCenter({ restaurantId }: { restaurantId: string }) {
        ...
        <div className="auditTable">
          {auditRows.map((row) => (
            <article key={row.id}>
              ...
              <small>{identities[row.changed_by || ""] || (row.changed_by ? "Usuario anterior" : "Sistema")} · {new Date(row.changed_at).toLocaleString("es-GT")}</small>
            </article>
          ))}
          {!auditRows.length && (
            <div className="empty">
