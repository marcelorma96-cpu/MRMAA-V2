...
export function EnhancedSettings({
  ...
  }, [restaurantId, setPreferences, onAreasChange]);
  useEffect(() => {
    load();
  }, [load]);
          ...
          {tab === "reservaciones" && (
            <div className="automationList">
              <section className="moduleCard"><h3>Áreas para reservaciones</h3>
                ...
                <div className="compactList">{reservationAreas.map(a=><article key={a.id}><b>{a.name}</b><button type="button" onClick={()=>{setReservationAreaEdit(a.id);setReservationAreaName(a.name);}}>Editar</button></article>)}</div>
